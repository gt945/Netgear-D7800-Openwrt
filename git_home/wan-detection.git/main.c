...
int main(int argc, const char **argv)
{
	...
	strcpy(country, config_get("dsl_wan_country"));
	strcpy(isp, config_get("dsl_wan_isp"));
	if (strcmp(country, "UK") == 0 && strcmp(isp, "Sky") == 0)
	{
		uk_sky_option61 = (char *)malloc(sizeof(char) * 128);
		snprintf(uk_sky_option61, sizeof(char) * 128, "%s", config_get("dsl_wan_ether_dhcp_option61"));
	}
