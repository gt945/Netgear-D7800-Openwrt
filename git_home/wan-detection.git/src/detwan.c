...
static void init_ctrl_rqst(struct pptp_start_ctrl_conn *req)
{
	...
	strcpy((char *)req->hostname, get_conf_string("detwan_pptp_hostname"));
...
static int PPTP_Detection(void)
{
	...
	char cmd[128];
	sprintf(cmd, "/sbin/route add -net %s netmask 255.255.255.255 %s", get_conf_string("detwan_pptp_server"), wan_if_name);
	system(cmd);
	sprintf(cmd, "/usr/sbin/iptables -t mangle -I PREROUTING -i %s -s %s -d %s -p tcp --sport %d -j ACCEPT", wan_if_name, get_conf_string("detwan_pptp_server"), get_conf_string("detwan_pptp_ip"), PPTP_PORT);
	system(cmd);
...
{
	char *file = get_conf_string("detwan_status_file");
