...
static int sys_unlink(pr_fs_t *fs, const char *path) {
  /* bug25349 
   * when file is accessed by other process, do not unlink it. 
   * */
  ...
  if(fcntl(fd, F_GETLK, &lock) < 0){
  	pr_log_pri(PR_LOG_NOTICE, "get lock info fail!");
  }
...
static int sys_open(pr_fh_t *fh, const char *path, int flags) {
  ...
  char *buf = "/shares";
  struct stat st;
  if(strncmp(path, buf, sizeof(buf)) || pr_fsio_stat(path, &st) < 0){
 	...
 	lock.l_whence = SEEK_SET;
  	...
  	if(fcntl(fd, F_SETLK, &lock) < 0){
 		...
 		return fd; 
  		...
  		pr_log_pri(PR_LOG_NOTICE, "lock sucess file %s after open it", path);
 		return fd; 
	}
