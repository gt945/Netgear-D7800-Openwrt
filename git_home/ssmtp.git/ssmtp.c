...
char **parse_options(int argc, char *argv[])
{
	...
	while(i < argc) {
		...
		while(argv[i][++j] != (char)NULL) {
			...
			// get the port from commandline parameter "-P port or -Pport"
			case 'P':
...
int get_ssmpt_port()
{
#define GET_SSMTP_PORT "config get email_port"
	FILE *fp;
	char port_buf[64] = {0};
	fp = popen(GET_SSMTP_PORT, "r");
	if (NULL == fp)
	{
		perror("popen failed to get port");
		exit(1);
	}

	if (fgets(port_buf, sizeof(port_buf), fp) != NULL)
		if ('\n' == port_buf[strlen(port_buf)-1])
			port_buf[strlen(port_buf)-1] = '\0';
	pclose(fp);
	//__nprintf("the port used by ssmtp is %d \n", atoi(port_buf));

	return atoi(port_buf);
}

void __nprintf(char *format, ...)
{
	va_list ap;
...
//	port = get_ssmpt_port();
	...
	/* port got from the commandlin option, the default is 25 */
