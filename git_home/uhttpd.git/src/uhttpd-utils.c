...
int uh_cgi_auth_check(
	...
	if( (plen >= rlen) && !strncasecmp(pi->name, cl->server->conf->cgi_prefix, rlen) )
	{
				...
				config_match("http_username", user) && config_match("http_passwd", pass))
		{
			...
			config_match("guest_enable", "1") && config_match("http_guestname", user) && config_match("http_guestpwd", pass))
		{
			if(strstr(pi->name, "/genie.cgi"))
				ret = AUTH_OK;
			else
...
int is_router_domain_addr(char *httphost, char *lan_ip, char *enable_ddns, char *ddns_host)
{
	if ( httphost == NULL )
		return 0;
                ...
                return 0;
	else if(strcasecmp(httphost, lan_ip)==0)
                return 0;
	else if(strcasecmp(enable_ddns, "1") == 0 && strncasecmp(httphost,ddns_host,strlen(ddns_host))==0)
		return 0;
        else
                return 1;
}
...
char *check_url(char *remote_addr, char *httphost, char *url)
{
	char *lan_ip, *lan_mask;
	char *enable_ddns, *ddns_host;

	lan_ip = strdup(config_get("lan_ipaddr"));
	lan_mask = strdup(config_get("lan_netmask"));
	enable_ddns = strdup(config_get("endis_ddns"));
	ddns_host = strdup(config_get("sysDNSHost"));
	...
	if(is_ipv6_domain(remote_addr)==0
			...
			&& is_router_domain_addr(httphost, lan_ip, enable_ddns, ddns_host)==1)
	{
		url = "/change_domain.htm";
	}
	return url;
}
