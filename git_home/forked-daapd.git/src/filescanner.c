...
/*
 * Read "remote_pairing_file" option in configuration file and return its
 * value.
 *
 * @return  value of "remote_pairing_file" option in configuration file.
 */
static char *
get_remote_pairing_file_path(void)
{
  cfg_t *lib_section = cfg_getsec(cfg, "library");

  return cfg_getstr(lib_section, "remote_pairing_file");
}

/*
 * Check specified .remote file. If the file exists, update internally stored
 * Remote pairing info according to the .remote file. Then, the .remote file
 * is removed.
 *
 * @param from_file  Path to .remote file containing new Remote pairing info
 * @return           0: OK;  1: Something wrong
 */
static int
update_remote_pairing_info(char *from_file)
{
  if (from_file == NULL)
    return 0;

  if (access(from_file, R_OK) == 0)
    {
      remote_pairing_read_pin(from_file);
      remove(from_file);
    }
  else if (errno != ENOENT)
    {
      DPRINTF(E_FATAL, L_SCAN, "Could not read Remote pairing file %s: %s\n",
...
{
  int is_bulkscan;
  char *remote_pairing_file;

  is_bulkscan = (flags & F_SCAN_BULK);

  remote_pairing_file = get_remote_pairing_file_path();
  update_remote_pairing_info(remote_pairing_file);

  /*
   * In case remote_pairing_file which has been already removed is read
   * again.
   */
  if (remote_pairing_file != NULL
      && strcmp(file, remote_pairing_file) == 0)
    {
      return;
    }
...
bulk_scan(int flags)
{
  ...
  for (i = 0; i < ndirs; i++)
      ...
      if (!deref)
	{
      ...
      update_remote_pairing_info(get_remote_pairing_file_path());
