...
#define ETH_CABLE_FILE	"/tmp/port_status"
//**** Fixed bug 54604 DUT can't send NTP sync package to get time by ADSL/VDSL****
...
{
	char *if_dsl;
	if_dsl = config_get("dsl_wan_preference");
	if(*if_dsl == '1'){
		return readc(DSL_CABLE_FILE) == '1';
	}
	else if(*if_dsl == '2'){
		return readc(ETH_CABLE_FILE) == '1';
	}
	else
		return 0;
}
...
static int net_verified(int *modep)
{
	...
	if (!strcmp(p, "pppoe") ||!strcmp(p, "pptp") || !strcmp(p, "pppoa") || !strcmp(p, "ipoa")) {
		mode = NET_PPP;
		alive = ppp_up();
