...
int daylight_saving_setting(void)
{
	...
	if (fp != NULL)
	{
		time(&now);
		tm = localtime(&now);
		if ((dst_enable != 0) && (0 == set_flag) && (dst_index != 2) && (dst_index != 3) && (dst_index != 4) && (dst_index != 6) && (dst_index != 7) && (dst_index != 9))
		{
	...
	if ((dst_enable != 0) && (dst_changed != 0))
	{
		time(&now);
		tm = localtime(&now);
		if ((0 == set_flag) && (dst_index != 2) && (dst_index != 3) && (dst_index != 4) && (dst_index != 6) && (dst_index != 7) && (dst_index != 9))
		{
			year = tm->tm_year+1900;
			month = tm->tm_mon+1;
			date = tm->tm_mday;
	...
	else if ((0 == dst_enable) && (dst_changed != 0))
	{
		time(&now);
		tm = localtime(&now);
		if ((0 == set_flag) && (dst_index != 2) && (dst_index != 3) && (dst_index != 4) && (dst_index != 6) && (dst_index != 7) && (dst_index != 9))
...
int main(int argc, char *argv[]) {
	...
	for (;;) {
		c = getopt(argc, argv, "c:"DEBUG_OPTION BOOT_RELAY_OPTION "h:i:p:b:w:n:a:j:u:x:z:m:s");
		if (c == EOF) break;
		switch (c) {
			case 'c':
			...
			case 'a':
				...
				break;
			case 'j':
				dst_enable = atoi(optarg);
				break;
			case 'u':
				dst_changed = atoi(optarg);
				break;
			case 'x':
				dst_index = atoi(optarg);
				break;
			case 'z':
	...
	/*set global variables for daylight saving time*/
		if (dst_changed == 1) //whether ntpadjust(dst enable or disable) is changed
			set_flag = 0;
