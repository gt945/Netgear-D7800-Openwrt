...
void usage(void)
{
	fprintf(stderr,	"Usage: netscan	<option> ...\n"
			"<options>:\n"
			"	-f <file>	(optional) appconfig file.\n"
...
{
	char *conf_file = "/tmp/etc/netscan.conf";
	int opt;
	int arp_sock;
	int bios_sock;
	int max_sock;
	...
	while ((opt = getopt(argc, argv, "f:lh")) != -1) {
		switch (opt) {
		case 'f':
			conf_file = optarg;
			break;
