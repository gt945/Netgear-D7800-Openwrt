...
void update_streamboost_info(int state, uint8 *mac, struct in_addr ip, char *name,
							netgear_devtype_t type, double down, double up, 
							long long epoch, int priority);
...
void get_streamboost_nodes_info(int state)
{
	...
	fp = popen("/www/cgi-bin/ozker |"
				"sed \'1,2d\' |"
			   	"jq \'.nodes[] |"
			   	".Pipeline.mac_addr, .Pipeline.ip_addr, .Pipeline.name,"
			   	".Pipeline.type, .Pipeline.default_prio, .Pipeline.down,"
			   	".Pipeline.up, .Pipeline.epoch, .UI.priority\'",
				"r");
	if (fp) {
		while (1) {
			down = 0;
			...
			if (ip.s_addr != 0)
				update_streamboost_info(state, mac.ether_addr_octet, ip, name,
										type, (double)down, (double)up, epoch,
									   	priority);
		}
		pclose(fp);
	}
...
static void get_dhcp_host(char host[], struct in_addr ip, int *isrepl)
{
	...
	host[0] = '\0';
	ipstr = inet_ntoa(ip);
	dhcp_list_file = get_value("dhcp_list_file");
...
#if SUPPORT_STREAMBOOST
void update_streamboost_info(int state, uint8 *mac, struct in_addr ip, 
							char *name, netgear_devtype_t type, double down,
						   	double up, long long epoch, int priority)
{
	uint32 i;
	struct arp_struct *u;
...
int open_arp_socket(struct sockaddr *me)
{
	...
	arp_ifname = get_value("arp_ifname");
...
int is_alive_wirless_client(uint8 *mac, char *wlan_sta_file)
{
	char mac_tmp[32];
	FILE *tfp;
...
void show_arp_table(void)
{
	...
	arp_file = get_value("arp_file");
	fp = fopen(arp_file, "w");
	if (fp == 0) 
		return;

	wlan_sta_file = get_value("wlan_sta_file");
	sprintf(cmd, "wlan stainfo >%s", wlan_sta_file);
	system(cmd);
	
	for (i = 0; i < (NEIGH_HASHMASK + 1); i++) {
		for (pprev = &arp_tbl[i], u = *pprev; u; ) {
			if (u->active == 0) {
				if(is_alive_wirless_client(u->mac, wlan_sta_file)){
					u->active = 1;
				}
				else{
