...
static int build_mac_hdr(struct ethhdr *eh)
{
	...
	if (config_match("dsl_wan_type", "adsl") && config_match("dsl_wan_preference", "1"))
	{
		char wan2_iface[12] = {0};
		strcpy(wan2_iface, config_get("real_wan2_ifname"));
		get_if_mac(src, wan2_iface);
	}
	else
...
static int build_bt_igmp(char *igmp_buf, int size, char *bt_igmp)
{
	...
	if (config_match("dsl_wan_isp", "PlusNet"))
		iph->saddr = 0x0116160A;
	else
...
int pcap_ppp()
{
    ...
    if(!strcmp(wan_proto, "pppoe") || !strcmp(wan_proto, "l2tp") || !strcmp(wan_proto, "pptp") || !strcmp(wan_proto, "pppoa"))
	device = PPP_IF;
    else
	device = WAN_IF;
    char errbuf[1024];
    ...
    */
	...
	if (config_match("dsl_wan_type", "adsl") && config_match("dsl_wan_preference", "1"))
	{
		char wan2_iface[12] = {0};
		strcpy(wan2_iface, config_get("real_wan2_ifname"));
		strcpy(req.ifr_name, wan2_iface);
	}
	else
