...
if [ "X$wanlan_capture" = "X1" ]; then 
	...
	if [ "$(config get dsl_wan_preference)" = "1" ]; then
		wan_if="ptm0"
		[ "$(config get dsl_wan_type)" = "adsl" ] && wan_if="nas0"
	elif [ "$(config get dsl_wan_preference)" = "2" ]; then
		wan_if="ethwan"
	fi
	[ "$(config get dsl_wan_preference)" = "1"
	if [ "X$store_locate" = "X1" -a "X$dist_path" != "X" ]; then
		echo "Save capture lan/wan packet in usb storage, lan:$lan_if, wan:$wan_if." > /dev/console
		mkdir $dist_path/Capture
		tcpdump -i $lan_if -s 0 -W 1 -w $dist_path/Capture/lan.pcap -C 100 &
		tcpdump -i $wan_if -s 0 -W 1 -w $dist_path/Capture/wan.pcap -C 100 &
	else
		echo "Save capture lan/wan packet in SDRAM tmp dir,  lan:$lan_if, wan:$wan_if." > /dev/console
		tcpdump -i $lan_if -s 0 -W 1 -w /tmp/lan.pcap -C 5 &
		tcpdump -i $wan_if  -s 0 -W 1 -w /tmp/wan.pcap -C 5 &
	fi
fi
