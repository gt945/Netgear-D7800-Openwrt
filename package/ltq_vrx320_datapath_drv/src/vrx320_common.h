#ifndef __VRX320_COMMON_H__
#define __VRX320_COMMON_H__
...
#define SOC_ACCESS_VRX218_ADDR(addr, base)      ((volatile unsigned int *)((base) + ((unsigned int)(addr) & 0x007FFFFF)))
...
#define SOC_US_FASTPATH_DES_BASE_VIRT    g_host_desc_base.us_fastpath_des_base_virt
...
#define SOC_US_CPUPATH_DES_BASE_VIRT     g_host_desc_base.us_cpupath_des_base_virt
...
#define SOC_DS_DES_BASE_VIRT             g_host_desc_base.ds_des_base_virt
#define SOC_DS_DES_NUM              64          //  max: 32
#define SOC_ATM_DS_DES_NUM          32          //  max: 32 for ATM
#define SOC_ATM_DS_DES_NUM_MAX      64
#define SOC_DS_OAM_DES_BASE         g_host_desc_base.ds_oam_des_base        //  chiptest: (0x1E180000 + (0x8980 * 4)
#define SOC_DS_OAM_DES_BASE_VIRT         g_host_desc_base.ds_oam_des_base_virt
#define SOC_DS_OAM_DES_NUM          32          //  max: 32
#define SOC_DS_OAM_DES_NUM_MAX      32

#ifdef AKRONITE_BOARD
#define  __DDR_BASE		0x347B
#endif
/*
 * ####################################
 *              Data Type
 ...
 * ####################################
 */
extern int print_fw_ver(struct seq_file *seq, struct fw_ver_id ver);
...
#else
#define dword_mem_write(__dst_addr, __src_addr, no_of_bytes) do{\
        printk("####%s:%d src=%#x dst=%#x####\n", __func__, __LINE__, (__src_addr),(__dst_addr)); \
        my_dword_mem_write(__dst_addr, __src_addr, no_of_bytes);\
}while(0)
#endif
...
extern int xet_phy_wan_port(uint32_t port, uint32_t *flags, uint32_t wanmode, int xmode);
#define VRX320_VIRT_BASE1_PHY 0x32000000
#define VRX320_VIRT_BASE1_VIRT 0xf1000000


extern int handle_rx_interrupt(void);
extern int32_t get_qid_by_priority(uint32_t port, uint32_t priority);
unsigned int showtime_exit_callback(void);
unsigned int showtime_init_callback(void);
