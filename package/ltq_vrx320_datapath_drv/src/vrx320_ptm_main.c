...
#include "vrx_ppe.h"
#include "vrx320_ppe_ptm_tc_ds.h"
#include "vrx320_ppe_fw_ds.h"
#include "vrx320_ppe_bonding_ds.h"
#include "unified_qos_ds_be.h"
#include "vrx320_e1_addr_def.h"
#include "vrx320_ppe_fw_const.h"
#include "vrx320_e1.h"
#include "vrx320_pp32_1.h"
...
int  g_poll_interval_ms = 100;
extern int __init ppa_driver_init(void);
extern void __exit ppa_driver_exit(void);

module_param(g_poll_interval_ms, int, 0644);
...
static ssize_t dsl_proc_write_status(struct file *, const char __user *, size_t,
				     loff_t *);
...
static int proc_read_showtime_seq_open(struct inode *inode, struct file *file);
...
static int proc_read_nsspath(struct seq_file *seq, void *v);
static int proc_read_nsspath_seq_open(struct inode *inode, struct file *file);
static ssize_t proc_write_nsspath(struct file *file, const char __user * buf, size_t count, loff_t * data);
#endif

static int proc_read_bonding_rx_poll(struct seq_file *seq, void *v);
static int proc_read_bonding_rx_poll_seq_open(struct inode *inode, struct file *file);
static ssize_t proc_write_bonding_rx_poll(struct file *file, const char __user * buf, size_t count, loff_t * data);
...
int g_dsl_bonding = 0;		//  default: single line
int g_queue_gamma_map[4] = { 0 };
static int g_eth_prio_queue_map[8][8];
static int g_wan_queue_class_map[8];
static unsigned int g_wan_itf = 3 << 6;

extern int g_showtime[2];

#ifdef QCA_NSS_REDIRECT
extern int g_use_nss_path;
extern struct net_device *g_ptm_net_dev[1];
extern void vrx320_nss_redirect_register(struct net_device *dev);
extern void vrx320_nss_redirect_unregister(void);
#endif
#ifdef VRX320_DEBUG
extern unsigned int g_intr_cnt, g_fake_isr, g_repoll_cnt, g_desc_reown_cnt;
extern unsigned int g_wd_min, g_wd_max, g_wd_avg, g_wd_cnt;
extern unsigned int g_tx_nodesc_cnt;
#endif
extern int g_bonding_rx_poll;
...
static ltq_pcie_ep_dev_t g_vrx218_dev[2] = { {0} };
static int g_vrx218_idx[2] = { -1, -1 };

//  pointer to device in single line case and upstream bonding master in bonding case
ltq_pcie_ep_dev_t *const g_us_vrx218_dev = &g_vrx218_dev[VRX218_US_BONDING_MASTER];
...
static ltq_pcie_ep_dev_t *g_ds_vrx218_dev = NULL;
//  pointer for random device access (usually via proc filesystem)
static ltq_pcie_ep_dev_t *g_cur_vrx218_dev = &g_vrx218_dev[VRX218_US_BONDING_MASTER];
...
static struct proc_entry_cfg g_proc_entry[] = {
	...
	{NULL, "showtime", 0, NULL, NULL, 1},
#ifdef QCA_NSS_REDIRECT
	{NULL, "nss_path", 0, NULL, NULL, 1},
#endif
	{NULL, "bonding_rx_poll", 0, NULL, NULL, 1},
...
static int init_local_variables(void)
{
	...
	for (i = 0; i < NUM_ENTITY(g_queue_gamma_map); i++) {
		g_queue_gamma_map[i] =
		    queue_gamma_map[i] & ((1 << /* g_wanqos_en */ 8) - 1);
		for (j = 0; j < i; j++)
...
static void set_vrx218_ppe_clk(unsigned int clk, unsigned int base)
{
	...
	case 288:
		*CGU_CLKFSR(base) =
		    (*CGU_CLKFSR(base) & ~0x00070000) | 0x00020000;
		break;
	case 432:
		*CGU_CLKFSR(base) =
		    (*CGU_CLKFSR(base) & ~0x00070000) | 0x00010000;
		break;
	}
}
 ...
 */
static int pp32_download_code(const ltq_pcie_ep_dev_t * dev, int pp32,
			      const u32 * code_src, unsigned int code_dword_len,
			      const u32 * data_src, unsigned int data_dword_len)
{
	...
	if (code_dword_len <= CDM_CODE_MEMORYn_DWLEN(0))
		...
		set = 0x00;
	dbg("%s: addr(CDM_CFG) = 0x%08x, clr = 0x%02x, set 0x%02x",
	    __FUNCTION__, (unsigned int)CDM_CFG(dev->membase), clr, set);
	IFX_REG_W32_MASK(clr, set, CDM_CFG(dev->membase));

	/*  clear code memory   */
	dest =
	    (volatile unsigned int *)
	    SOC_ACCESS_VRX218_ADDR(CDM_CODE_MEMORY(pp32, 0), dev->membase);
	...
	/*  copy code   */
	dest = (volatile unsigned int *) SOC_ACCESS_VRX218_ADDR(CDM_CODE_MEMORY(pp32, 0), dev->membase);
	while (code_dword_len-- > 0)
		IFX_REG_W32((*code_src++), dest++);

	/*  copy data   */
	dest = (volatile unsigned int *) SOC_ACCESS_VRX218_ADDR(CDM_DATA_MEMORY(pp32, 0), dev->membase);
...
static int pp32_load(const ltq_pcie_ep_dev_t * dev)
{
	int ret;

	/*  download firmware   */
	ret = pp32_download_code(dev, 0, vrx218_e1_fw_code, NUM_ENTITY(vrx218_e1_fw_code), 
		vrx218_e1_fw_data, NUM_ENTITY(vrx218_e1_fw_data));
	if (ret != IFX_SUCCESS)
		return ret;

	pp32_download_code(dev, 1, vrx218_pp32_1_fw_code, NUM_ENTITY(vrx218_pp32_1_fw_code),
			   vrx218_pp32_1_fw_data, NUM_ENTITY(vrx218_pp32_1_fw_data));

	return IFX_SUCCESS;
}

static int pp32_start(const ltq_pcie_ep_dev_t * dev)
{
	unsigned int mask = 0x00010001;

	/*  run PP32    */
	IFX_REG_W32_MASK(mask, 0, PP32_FREEZE(dev->membase));
...
static void pp32_stop(const ltq_pcie_ep_dev_t * dev)
{
	unsigned int mask = 0x00010001;

	/*  halt PP32   */
	IFX_REG_W32_MASK(0, mask, PP32_FREEZE(dev->membase));
}
...
//  vrx218_ppe_ptm_init.c
	...
	int port = base == VRX320_VIRT_BASE1_VIRT ? 1 : 0;
	int i;
...
//  vrx218_ppe_ptm_init.c
extern void vrx218_ppe_ptm_init(const ltq_pcie_ep_dev_t * dev, const ltq_pcie_ep_dev_t * peer_dev,
				int is_bonding, unsigned int vrx218_id, unsigned long sync_buf, int lle_in_sb,
				int cdma_write_data_en);
extern void __exit vrx218_ppe_ptm_fw_exit(const ltq_pcie_ep_dev_t * dev, int is_bonding,
					  unsigned int vrx218_id);
extern void __exit vrx218_ppe_ptm_free_mem(const ltq_pcie_ep_dev_t * dev, int is_bonding,
					   unsigned int vrx218_id);
//  vrx218_datapath.c
extern int vrx218_ptm_datapath_init(const ltq_pcie_ep_dev_t * p_vrx218_dev_us, const ltq_pcie_ep_dev_t * p_vrx218_dev_ds);
extern void vrx218_ptm_datapath_exit(void);

static int __init init_vrx218_module(void)
{
	printk("port        = %d\n",
	       (unsigned int)g_vrx218_dev->membase == VRX320_VIRT_BASE1_VIRT ? 1 : 0);
	printk("irq         = %u\n", g_vrx218_dev->irq);
	 ...
	 *  init hardware
	 */
	reset_ppe((unsigned int)g_vrx218_dev->membase);	/* AKRONITE FIX */
	/*  activate VRX218 */
	enable_vrx218_ppe_ema((unsigned int)g_vrx218_dev->membase);
	/*  set VRX218 PPE clock 432MHz */
	set_vrx218_ppe_clk(432, (unsigned int)g_vrx218_dev->membase);
	/*  clear and disable mailbox   */
	*MBOX_IGU0_ISRC(g_vrx218_dev->membase) = 0xFFFFFFFF;
	*MBOX_IGU1_ISRC(g_vrx218_dev->membase) = 0xFFFFFFFF;
	*MBOX_IGU0_IER(g_vrx218_dev->membase) = 0x00;
	*MBOX_IGU1_IER(g_vrx218_dev->membase) = 0x00;
	/*  freeze PP32 */
	pp32_stop(g_vrx218_dev);
	vrx218_ppe_ptm_init(g_vrx218_dev, NULL, 0 /* is_bonding */ ,
			    0 /* vrx218_id */ , 0 /* sync_buf */ ,
			    0 /* lle_in_sb */ , 0 /* cdma_write_data_en */ );
	...
	setup_dfe_loopback((unsigned int)g_vrx218_dev->membase);
	...
	*MBOX_IGU0_IER(g_vrx218_dev->membase) = 0x00;	/* not ready to implement flow control yet */
	/*
	 ...
	 */
	/*  Bit 5 - DS packet interrupt */
	*MBOX_IGU1_IER(g_vrx218_dev->membase) = 0x2D;
...
static void __exit exit_vrx218_module(void)
{
	...
	vrx218_ppe_ptm_fw_exit(g_vrx218_dev, 0 /* is_bonding */ ,
			       0 /* vrx218_id */ );
	udelay(100);
...
static int __init init_vrx218_bonding_module(void)
{
	...
	printk("port        = %d\n",
	       (unsigned int)g_us_vrx218_dev->membase == VRX320_VIRT_BASE1_VIRT ? 1 : 0);
	printk("irq         = %u\n", g_us_vrx218_dev->irq);
	 ...
	 *  init hardware
	 */
	reset_ppe((unsigned int)g_us_vrx218_dev->membase);
	/*  activate VRX218 */
	enable_vrx218_ppe_ema((unsigned int)g_us_vrx218_dev->membase);
	/*  set VRX218 PPE clock 432MHz */
	set_vrx218_ppe_clk(432, (unsigned int)g_us_vrx218_dev->membase);
	/*  clear and disable mailbox   */
	*MBOX_IGU0_ISRC(g_ds_vrx218_dev->membase) = 0xFFFFFFFF;
	*MBOX_IGU1_ISRC(g_ds_vrx218_dev->membase) = 0xFFFFFFFF;
	*MBOX_IGU0_IER(g_ds_vrx218_dev->membase) = 0x00;
	*MBOX_IGU1_IER(g_ds_vrx218_dev->membase) = 0x00;
	/*  freeze PP32 */
	pp32_stop(g_us_vrx218_dev);
	vrx218_ppe_ptm_init(g_us_vrx218_dev,
			    g_ds_vrx218_dev, 1 /* is_bonding */ ,
	...
	printk("port        = %d\n",
	       (unsigned int)g_ds_vrx218_dev->membase == VRX320_VIRT_BASE1_VIRT ? 1 : 0);
	printk("irq         = %u\n", g_ds_vrx218_dev->irq);
	printk("membase     = 0x%08x\n",
	 ...
	 *  init hardware
	 */
	reset_ppe((unsigned int)g_ds_vrx218_dev->membase);
	/*  activate VRX218 */
	enable_vrx218_ppe_ema((unsigned int)g_ds_vrx218_dev->membase);
	/*  set VRX218 PPE clock 432MHz */
	set_vrx218_ppe_clk(432, (unsigned long)g_ds_vrx218_dev->membase);
	/*  clear and disable mailbox   */
	*MBOX_IGU0_ISRC(g_us_vrx218_dev->membase) = 0xFFFFFFFF;
	*MBOX_IGU1_ISRC(g_us_vrx218_dev->membase) = 0xFFFFFFFF;
	*MBOX_IGU0_IER(g_us_vrx218_dev->membase) = 0x00;
	*MBOX_IGU1_IER(g_us_vrx218_dev->membase) = 0x00;
	/*  freeze PP32 */
	pp32_stop(g_ds_vrx218_dev);
	vrx218_ppe_ptm_init(g_ds_vrx218_dev,
			    g_us_vrx218_dev, 1 /* is_bonding */ ,
	...
	setup_dfe_loopback((unsigned int)g_us_vrx218_dev->membase);
	pp32_start(g_ds_vrx218_dev);
	setup_dfe_loopback((unsigned long)g_ds_vrx218_dev->membase);
	 ...
	 */
	/*  Bit 5 - Peer-to-peer-link-state */
	*MBOX_IGU0_IER(g_us_vrx218_dev->membase) = 0x00;	/* not ready to implement flow control yet */
	*MBOX_IGU1_IER(g_us_vrx218_dev->membase) = 0x3D;	/* not ready to implement swap queue, but enable DS */
	*MBOX_IGU0_IER(g_ds_vrx218_dev->membase) = 0x00;	/* not ready to implement flow control yet */
	*MBOX_IGU1_IER(g_ds_vrx218_dev->membase) = 0x3D;	/* not ready to implement swap queue, but enable DS */
...
static void __exit exit_vrx218_bonding_module(void)
{
	...
	vrx218_ppe_ptm_fw_exit(g_us_vrx218_dev, 1 /* is_bonding */ ,
			       VRX218_US_BONDING_MASTER /* vrx218_id */ );
	vrx218_ppe_ptm_fw_exit(g_ds_vrx218_dev, 1 /* is_bonding */ ,
			       VRX218_DS_BONDING_MASTER /* vrx218_id */ );
	udelay(100);
	...
	vrx218_ppe_ptm_free_mem(g_us_vrx218_dev, 1 /* is_bonding */ ,
				VRX218_US_BONDING_MASTER /* vrx218_id */ );
	vrx218_ppe_ptm_free_mem(g_ds_vrx218_dev, 1 /* is_bonding */ ,
				VRX218_DS_BONDING_MASTER /* vrx218_id */ );
}
...
static void switch_to_eth_trunk(void)
{
	...
	__switch_to_eth_thunk((unsigned long)g_us_vrx218_dev->membase);
	__switch_to_eth_thunk((unsigned long)g_ds_vrx218_dev->membase);
...
static void switch_to_ptmtc_bonding(void)
{
	...
	local_irq_save(sys_flag);
	__switch_to_ptmtc_bonding((unsigned long)g_us_vrx218_dev->membase);
	__switch_to_ptmtc_bonding((unsigned long)g_ds_vrx218_dev->membase);
	local_irq_restore(sys_flag);
}
...
{
	__update_max_delay((unsigned long)g_us_vrx218_dev->membase, line);
	__update_max_delay((unsigned long)g_ds_vrx218_dev->membase, line);
}
...
static int dsl_proc_read_status(struct seq_file *seq, void *v)
{
	...
	if (g_dsl_bonding)
		seq_printf(seq,
			   BOND_CONF(g_vrx218_dev->membase)->bond_mode ==
			   L1_PTM_TC_BOND_MODE ? "Bonding Mode: PTM TC\n" :
...
static ssize_t dsl_proc_write_status(struct file *file, const char __user * buf,
				     size_t count, loff_t * data)
{
	...
	char *env_var_str[] =
	    { "help", "DSL_LINE_NUMBER", "DSL_TC_LAYER_STATUS",
  "DSL_EFM_TC_CONFIG_US", "DSL_BONDING_STATUS", "DSL_INTERFACE_STATUS",
  "DSL_DATARATE_US_BC", "DSL_DATARATE_DS_BC", "MAX_DELAY" };
	int line = -1, down = 0, upstr = 0, downstr = 0;
	int datarate_us[BEARER_CHANNEL_PER_LINE] =
	    { 0 }, datarate_ds[BEARER_CHANNEL_PER_LINE] = {
	0};
	unsigned int num, bc;
	int i;
	...
	while (get_token(&p1, &p2, &len, &colon)) {
		for (i = 0;
		     i < NUM_ENTITY(env_var_str)
		     && strncasecmp(p1, env_var_str[i],
				    strlen(env_var_str[i])) != 0; i++) ;
		switch (i) {
		case 0:	//  help
			printk("Usage:\n");
		...
		case 5:	//  DSL_INTERFACE_STATUS
			if (line >= 0) {
				p1 = p2;
				colon = 1;
				if (get_token(&p1, &p2, &len, &colon) && strcasecmp(p1, "down") == 0
	...
	if (line >= 0) {
		if (down) {
			for (i = 0, num = 0; i < LINE_NUMBER;
			     num += g_line_showtime[i], i++) ;
			if (num == 0) {
				g_showtime[0] = 0;	//  equivalent to dsl_showtime_exit
			...
			if (upstr) {
				...
				g_showtime[0] = 1;	//  equivalent to dsl_showtime_enter
			}
			if (downstr) {
				for (i = 0; i < BEARER_CHANNEL_PER_LINE; i++)
					if (datarate_ds[i] != 0)
...
static struct file_operations g_proc_file_showtime_seq_fops = {
	.owner = THIS_MODULE,
	.open = proc_read_showtime_seq_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
static int proc_read_showtime(struct seq_file *seq, void *v)
{
	seq_printf(seq, "%d %d\n", g_showtime[0], g_showtime[1]);

#ifdef VRX320_DEBUG
	seq_printf(seq, "intr_cnt:%u fake_isr:%u repoll_cnt:%u reown_cnt:%u tx_nodesc_cnt:%u\n", 
							g_intr_cnt, g_fake_isr, g_repoll_cnt, g_desc_reown_cnt, g_tx_nodesc_cnt);
	seq_printf(seq, "wd_min:%u wd_max:%u wd_avg:%u wd_cnt:%u avg:%u\n", g_wd_min, 
							g_wd_max, g_wd_avg, g_wd_cnt, g_wd_cnt?g_wd_avg/g_wd_cnt:0);
#endif
	return 0;
}
...
static int proc_read_showtime_seq_open(struct inode *inode, struct file *file)
{
	return single_open(file, proc_read_showtime, NULL);
}

#ifdef QCA_NSS_REDIRECT
static struct file_operations g_proc_file_nsspath_seq_fops = {
	.owner = THIS_MODULE,
	.open = proc_read_nsspath_seq_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.write = proc_write_nsspath,
	.release = single_release,
};
...
static int proc_read_nsspath_seq_open(struct inode *inode, struct file *file)
{
	return single_open(file, proc_read_nsspath, NULL);
}

static ssize_t proc_write_nsspath(struct file *file, const char __user * buf,
			      size_t count, loff_t * data)
{
	char str[1024];
	char *p;
	...
	use_nss_path = simple_strtol(p, NULL, 0);
	
	if (use_nss_path != g_use_nss_path) {
		switch (use_nss_path) {
		case 1:
...
static struct file_operations g_proc_file_bonding_rx_poll_seq_fops = {
	.owner = THIS_MODULE,
	.open = proc_read_bonding_rx_poll_seq_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.write = proc_write_bonding_rx_poll,
	.release = single_release,
};
...
static int proc_read_bonding_rx_poll_seq_open(struct inode *inode, struct file *file)
{
	return single_open(file, proc_read_bonding_rx_poll, NULL);
}

static ssize_t proc_write_bonding_rx_poll(struct file *file, const char __user * buf,
			      size_t count, loff_t * data)
{
	char str[1024];
	char *p;
	...
	g_bonding_rx_poll = simple_strtol(p, NULL, 0);
	return count;
}
...
static ssize_t proc_write_prio(struct file *file, const char __user * buf,
			      size_t count, loff_t * data)
{
	char local_buf[900];
	int len;
	...
	while ( get_token(&p1, &p2, &len, &colon) ) {
		if ( stricmp(p1, "help") == 0 ) {
			printk("echo <ptm0> prio xx queue xx [prio xx queue xx] > /proc/vrx320/prio\n");
			break;
			...
			if ( stricmp(p1, "p") == 0 || stricmp(p1, "prio") == 0 ) {
				...
				dbg("queue = %d", queue);
				if ( port >= 0 && port < NUM_ENTITY(g_eth_prio_queue_map) && queue >= 0 && queue < (g_wan_itf & (1 << port)) ) {
					if ( prio >= 0 )
						g_eth_prio_queue_map[port][prio] = queue;
				} else {
					err("queue (%d) is out of range 0 - %d", queue, 7); 
				}
			} else {
...
static void proc_file_create(void)
{
	...
	for (i = 0; i < NUM_ENTITY(g_proc_entry); i++) {
		...
		if (g_proc_entry[i].is_dir) {
			...
			if (g_proc_entry[i].proc_entry == NULL)
				printk
				    ("%s: Create proc directory \"%s\" failed!\n",
	...
	for (i = 0; i < ARRAY_SIZE(g_proc_entry); i++) {
		if (strcmp(g_proc_entry[i].name, "dsl_tc") == 0)
			res =
			    proc_create("status", S_IRUGO | S_IWUSR,
					g_proc_entry[i].proc_entry,
					&g_proc_file_status_seq_fops);
	}
	res = proc_create("ver", S_IRUGO, parent_proc_entry,
			&g_proc_file_ver_seq_fops);
	res = proc_create("dbg", S_IRUGO | S_IWUSR, parent_proc_entry,
			&g_proc_file_dbg_seq_fops);
	res = proc_create("mem", S_IWUSR, parent_proc_entry,
			&g_proc_file_mem_seq_fops);
	res = proc_create("pp32", S_IRUGO | S_IWUSR, parent_proc_entry,
			&g_proc_file_pp32_seq_fops);
	res = proc_create("qos", S_IRUGO, parent_proc_entry,
			&g_proc_file_qos_seq_fops);
	res = proc_create("tfwdbg", S_IWUSR, parent_proc_entry,
			&g_proc_file_fwdbg_seq_fops);
	res = proc_create("dev", S_IRUGO | S_IWUSR, parent_proc_entry,
			&g_proc_file_dev_seq_fops);
	res = proc_create("mib", S_IRUGO | S_IWUSR, parent_proc_entry,
			&g_proc_file_wanmib_seq_fops);
#if defined(DEBUG_BONDING_PROC) && DEBUG_BONDING_PROC
	res = proc_create("bonding", S_IRUGO | S_IWUSR, parent_proc_entry,
			&g_proc_file_bonding_seq_fops);
	res = proc_create("bondingmib", S_IRUGO | S_IWUSR, parent_proc_entry,
			&g_proc_file_bondingmib_seq_fops);
#endif
	res = proc_create("pciereset", S_IRUGO | S_IWUSR, parent_proc_entry,
			&g_proc_file_pciereset_seq_fops);
	res = proc_create("showtime", S_IRUGO | S_IWUSR, parent_proc_entry,
			&g_proc_file_showtime_seq_fops);
#ifdef QCA_NSS_REDIRECT
	res = proc_create("nss_path", S_IRUGO | S_IWUSR, parent_proc_entry,
			&g_proc_file_nsspath_seq_fops);
#endif
	res = proc_create("bonding_rx_poll", S_IRUGO | S_IWUSR, parent_proc_entry,
			&g_proc_file_bonding_rx_poll_seq_fops);
	res = proc_create("prio", S_IRUGO | S_IWUSR, parent_proc_entry,
			&g_proc_file_prio_seq_fops);
}
...
static int proc_read_ver(struct seq_file *seq, void *v)
{
	...
	if (g_dsl_bonding) {
		...
		print_fw_ver(seq, *FW_VER_ID(g_us_vrx218_dev->membase));
		seq_printf(seq, "VRX318_DS_BONDING_MASTER");
		print_fw_ver(seq, *FW_VER_ID(g_ds_vrx218_dev->membase));
	} else {
		//  single line
		print_fw_ver(seq, *FW_VER_ID(g_cur_vrx218_dev->membase));
	}
			      ...
			      size_t count, loff_t * data)
{
	...
	if (strncasecmp(p, "enable", 6) == 0) {
		...
		printk ("echo <enable/disable> [err/dbg/assert/rx/tx/swap/all] > /proc/eth/vrx320/dbg\n");
	}
...
static int tokenize(char *in, char *sep, char **tokens, int max_token_num,
		    int *finished)
{
	int token_num = 0;
...
{
	const char *proc_file = "/proc/eth/vrx320/tfwdbg";
	const char *cmds[] = {
		"help",
		"qosqctxt",
		"dessync",
	...
	if (strncasecmp(tokens[0], "qosqctxt", strlen(tokens[0])) == 0) {
		...
		printk("echo dessync [CPU | FASTPATH | DS | OUTQ | LOCALQ | DSFRAGQ | USFRQGQ | USEDMARD | DSEDMAWR ] [qidx] > %s \n",
		     proc_file);
		printk("     :print vrx320 descriptor sync structure \n");
...
static void dump_qosqctxt(int qid)
{
	...
	qmib = WAN_QOS_MIB_TABLE(qid, g_cur_vrx218_dev->membase);
	...
	printk ("                  0x%08x/0x%08x        0x%08x/0x%08x       0x%08x/0x%08x       0x%08x/0x%08x\n",
	     qmib->rx_pkt_cnt, qmib->rx_byte_cnt, qmib->tx_pkt_cnt,
	     qmib->tx_byte_cnt, qmib->small_pkt_drop_cnt,
	     qmib->small_pkt_drop_byte_cnt, qmib->large_pkt_drop_cnt,
	     qmib->large_pkt_drop_byte_cnt);
...
{
	volatile desq_cfg_ctxt_t *des_sync_ctxt =
	    (volatile desq_cfg_ctxt_t *)SOC_ACCESS_VRX218_ADDR(SB_BUFFER(addr),
							       g_cur_vrx218_dev->
							       membase);
...
static void fwdbg_des_sync(char **tokens, int token_num)
{
	...
	if (token_num > 0) {
		for (i = 0; des_sync_path[i].name != NULL; i++) {
			if (strncasecmp (tokens[0], des_sync_path[i].name,
			     strlen(tokens[0])) == 0) {
				idx = i;
				break;
			}
	...
	if (idx != -1) {
		...
		if (i < 0 || i >= des_sync_path[idx].num) {
			for (i = 0; i < des_sync_path[idx].num; i++) {
				sprintf(ctxt_name, "%s%d",
					des_sync_path[idx].name, i);
				print_des_sync(ctxt_name,
					       (des_sync_path[idx].addr +
						i * 8));
			}
		} else {
			sprintf(ctxt_name, "%s%d", des_sync_path[idx].name, i);
			print_des_sync(ctxt_name,
				       (des_sync_path[idx].addr + i * 8));
		}
...
static void switch_to_ptmtc_bonding_mode(unsigned int base)
{
	...
	*SOC_ACCESS_VRX218_ADDR(VRX_PPE_PDMA_RX_MAX_LEN_REG, base) = 0x02040204;
	...
	for (i = 0; i < 4; i++) {
		dst_addr = (void *) SOC_ACCESS_VRX218_SB((__RX_GIF0_CFG_STATS_CFG +
					  (i * sizeof(struct rx_gamma_itf_cfg) / sizeof(unsigned int))), base);
...
static void fwdbg_bonding_mode_switch(char **tokens, int token_num)
{
	...
	if (strncasecmp(tokens[0], "L1", strlen(tokens[0])) == 0) {	//switch to TC-Bonding mode
		switch_to_ptmtc_bonding_mode((unsigned long)g_us_vrx218_dev->
					     membase);
		switch_to_ptmtc_bonding_mode((unsigned long)g_ds_vrx218_dev->
					     membase);
	} else if (strncasecmp(tokens[0], "L2", strlen(tokens[0])) == 0) {
		switch_to_eth_trunk_mode((unsigned int)g_us_vrx218_dev->
					 membase);
		switch_to_eth_trunk_mode((unsigned int)g_ds_vrx218_dev->
					 membase);
	} else {
		fw_dbg_start("help bdmdswitch");
	}
...
static void fwdbg_rx_bc_cfg(char **tokens, int token_num)
{
	...
	for (i = 0; i < 2; i++) {
		rx_bc_stat =
		    *(struct rx_bc_cfg *)
		    SOC_ACCESS_VRX218_SB((__RX_BC0_CFG_STATS_CFG +
					  (i * sizeof(struct rx_bc_cfg) / sizeof(unsigned int))),
					 g_cur_vrx218_dev->membase);
...
static void fwdbg_rd_rx_cb(char **tokens, int token_num)
{
	...
	unsigned int addr_base = (unsigned int)g_cur_vrx218_dev->membase;
	...
	for (i = start_pg, num = 0; num < page_num;
	     num++, i = (i + 1) % page_max_num) {
		...
		printk("cwid=%d [%-16s], cwer=%d, preempt=%d, short=%d\n",
		       p_ctrl->cwid, cwid_txt[p_ctrl->cwid], p_ctrl->cwer,
		       p_ctrl->preempt, p_ctrl->shrt);

		printk("       %08x %08x %08x %08x ", cw[1], cw[2], cw[3], cw[4]);
		printk("state=%d, bad=%d, ber=%-3d, spos=%-3d, ffbn=%-3d\n",
		       p_ctrl->state, p_ctrl->bad, p_ctrl->ber, p_ctrl->spos,
		       p_ctrl->ffbn);
...
static void fwdbg_rd_tx_cb(char **tokens, int token_num)
{
	...
	dbase =
	    (volatile unsigned int *)
	    SOC_ACCESS_VRX218_SB(FFSM_DBA(bc_id, addr_base)->dbase + 0x2000,
				 addr_base);
...
static void fwdbg_clr_cb(char **tokens, int token_num)
{
	...
	while (token_num > i) {
		switch (i) {
		case 0:	//get rx or tx
			if (strncasecmp(tokens[i], "tx", strlen(tokens[i])) ==
			    0) {
				is_rx = 2;
			} else {
				is_rx = 1;
...
static void fwdbg_arcjtag(char **tokens, int token_num)
{
	...
	base = (unsigned int)g_cur_vrx218_dev->membase;
...
static ssize_t proc_write_fwdbg(struct file *file, const char __user * buf,
				size_t count, loff_t * data)
{
	char str[1024];
	char *p;
...
static int proc_read_qos(struct seq_file *seq, void *v)
{
	...
	for (i = 0; i < VRX218_QOSQ_NUM; i++) {
		p = (volatile uint32_t *) sb_addr_to_fpi_addr_convert((unsigned long)(__QOSQ_MIB_BASE +
								i * (sizeof (qosq_mib_t) / sizeof (uint32_t))));
		qmib = (qosq_mib_t *) SOC_ACCESS_VRX218_ADDR(p, g_us_vrx218_dev-> membase);
		seq_printf(seq, "  [%d]:  %010u/%010u     %010u/%010u       %010u/%010u        %010u/%010u     @0x%x\n",
			   i, qmib->rx_pkt_cnt, qmib->rx_byte_cnt,
			   qmib->tx_pkt_cnt, qmib->tx_byte_cnt,
			   qmib->small_pkt_drop_cnt,
			   qmib->small_pkt_drop_byte_cnt,
			   qmib->large_pkt_drop_cnt,
			   qmib->large_pkt_drop_byte_cnt, (uint32_t) qmib);
	}
...
#ifdef UNUSED
static ssize_t proc_write_qos(struct file *file, const char __user * buf,
			      size_t count, loff_t * data)
{
	char local_buf[64];
	int len;
	...
	if (strcasecmp(p1, "clear") == 0) {
		for (i = 0; i < VRX218_QOSQ_NUM; i++) {
			p = (volatile unsigned long *) sb_addr_to_fpi_addr_convert((unsigned long)(__QOSQ_MIB_BASE +
							       i * (sizeof (qosq_mib_t) / sizeof (uint32_t))));
			qmib = (qosq_mib_t *) SOC_ACCESS_VRX218_ADDR(p, g_us_vrx218_dev-> membase);
...
{
	int len = 0;
	u32 base = (u32) g_cur_vrx218_dev->membase;
...
static ssize_t proc_write_ps(struct file *file, const char __user * buf,
			     size_t count, loff_t * data)
{
	char str[128];
	char *p;
...
static ssize_t proc_write_mem(struct file *file, const char __user * buf,
			      size_t count, loff_t * data)
{
	char *p1, *p2;
	int len;
	...
	colon = 1;
	while (get_token(&p1, &p2, &len, &colon)) {
		if (strcasecmp(p1, "w") == 0 || strcasecmp(p1, "write") == 0
		    || strcasecmp(p1, "r") == 0 || strcasecmp(p1, "read") == 0)
			break;
	...
	if (*p1 == 'w') {
		...
		p = (volatile unsigned long *)
		    sb_addr_to_fpi_addr_convert((unsigned long)p);
		p = (volatile unsigned long *)SOC_ACCESS_VRX218_ADDR(p, g_cur_vrx218_dev-> membase);
...
static int proc_read_pp32(struct seq_file *seq, void *v)
{
	...
	for (pp32 = 0; pp32 < 2; pp32++) {
		...
		if ((*PP32_FREEZE(g_cur_vrx218_dev->membase) &
		     ((1 << (pp32 << 4)))) != 0) {
			sprintf(str, "freezed");
			f_stopped = 1;
		} else
		    if (PP32_CPU_USER_STOPPED(g_cur_vrx218_dev->membase, pp32)
			|| PP32_CPU_USER_BREAKIN_RCV(g_cur_vrx218_dev->membase, pp32)
			|| PP32_CPU_USER_BREAKPOINT_MET(g_cur_vrx218_dev-> membase, pp32)) {
			strlength = 0;
			if (PP32_CPU_USER_STOPPED (g_cur_vrx218_dev->membase, pp32))
				strlength += sprintf(str + strlength, "stopped");
			if (PP32_CPU_USER_BREAKPOINT_MET
			    (g_cur_vrx218_dev->membase, pp32))
			...
			f_stopped = 1;
		} else if (PP32_CPU_CUR_PC(g_cur_vrx218_dev->membase, pp32) ==
			   PP32_CPU_CUR_PC(g_cur_vrx218_dev->membase, pp32)) {
			unsigned int pc_value[64] = { 0 };

			f_stopped = 1;
			for (i = 0; f_stopped && i < NUM_ENTITY(pc_value); i++) {
				pc_value[i] = PP32_CPU_CUR_PC(g_cur_vrx218_dev->membase, pp32);
				for (j = 0; j < i; j++)
		...
		if (!f_stopped)
			sprintf(str, "running");
		cur_context = PP32_BRK_CUR_CONTEXT(g_cur_vrx218_dev->membase, pp32);
		seq_printf(seq, "Context: %d, PC: 0x%04x, %s\n", cur_context,
			   PP32_CPU_CUR_PC(g_cur_vrx218_dev->membase, pp32), str);

		if (PP32_CPU_USER_BREAKPOINT_MET
		    (g_cur_vrx218_dev->membase, pp32)) {
			strlength = 0;
			if (PP32_BRK_PC_MET(g_cur_vrx218_dev->membase, pp32, 0))
				strlength += sprintf(str + strlength, "pc0");
			if (PP32_BRK_PC_MET(g_cur_vrx218_dev->membase, pp32, 1))
				strlength += sprintf(str + strlength, strlength ? " | pc1" : "pc1");
			if (PP32_BRK_DATA_ADDR_MET
			    (g_cur_vrx218_dev->membase, pp32, 0))
				strlength +=
				    sprintf(str + strlength, strlength ? " | daddr0" : "daddr0");
			if (PP32_BRK_DATA_ADDR_MET (g_cur_vrx218_dev->membase, pp32, 1))
				strlength += sprintf(str + strlength, strlength ? " | daddr1" : "daddr1");
			if (PP32_BRK_DATA_VALUE_RD_MET
			    (g_cur_vrx218_dev->membase, pp32, 0)) {
				strlength += sprintf(str + strlength, strlength ? " | rdval0" : "rdval0");
				if (PP32_BRK_DATA_VALUE_RD_LO_EQ
				    (g_cur_vrx218_dev->membase, pp32, 0)) {
					if (PP32_BRK_DATA_VALUE_RD_GT_EQ (g_cur_vrx218_dev->membase, pp32, 0))
			...
			if (PP32_BRK_DATA_VALUE_RD_MET (g_cur_vrx218_dev->membase, pp32, 1)) {
				strlength += sprintf(str + strlength, strlength ? " | rdval1" : "rdval1");
				if (PP32_BRK_DATA_VALUE_RD_LO_EQ (g_cur_vrx218_dev->membase, pp32, 1)) {
					if (PP32_BRK_DATA_VALUE_RD_GT_EQ (g_cur_vrx218_dev->membase, pp32, 1))
						strlength += sprintf(str + strlength, " ==");
					else
						strlength += sprintf(str + strlength, " <=");
				} else
			...
			if (PP32_BRK_DATA_VALUE_WR_MET (g_cur_vrx218_dev->membase, pp32, 0)) {
				strlength += sprintf(str + strlength, strlength ? " | wtval0" : "wtval0");
				if (PP32_BRK_DATA_VALUE_WR_LO_EQ (g_cur_vrx218_dev->membase, pp32, 0)) {
					if (PP32_BRK_DATA_VALUE_WR_GT_EQ (g_cur_vrx218_dev->membase, pp32, 0))
						strlength += sprintf(str + strlength, " ==");
					else
						strlength += sprintf(str + strlength, " <=");
				} else
			...
			if (PP32_BRK_DATA_VALUE_WR_MET (g_cur_vrx218_dev->membase, pp32, 1)) {
				strlength += sprintf(str + strlength,
					    strlength ? " | wtval1" : "wtval1");
				if (PP32_BRK_DATA_VALUE_WR_LO_EQ
				    (g_cur_vrx218_dev->membase, pp32, 1)) {
					if (PP32_BRK_DATA_VALUE_WR_GT_EQ (g_cur_vrx218_dev->membase, pp32, 1))
						strlength += sprintf(str + strlength, " ==");
					else
						strlength += sprintf(str + strlength, " <=");
				} else
		...
		if (f_stopped) {
			...
			for (i = 0; i < 4; i++) {
				for (j = 0; j < 4; j++)
					seq_printf(seq, "   %2d: %08x", i + j * 4, *PP32_GP_CONTEXTi_REGn
						   (g_cur_vrx218_dev->membase,
						    pp32, cur_context,
						    i + j * 4));
				seq_printf(seq, "\n");
		...
		seq_printf(seq, "break out on: break in - %s, stop - %s\n",
			   PP32_CTRL_OPT_BREAKOUT_ON_BREAKIN(g_cur_vrx218_dev->membase, pp32) ? stron :
			   stroff, PP32_CTRL_OPT_BREAKOUT_ON_STOP(g_cur_vrx218_dev->membase, pp32) ? stron : stroff);
		seq_printf(seq, "     stop on: break in - %s, break point - %s\n",
			   PP32_CTRL_OPT_STOP_ON_BREAKIN(g_cur_vrx218_dev-> membase,
							 pp32) ? stron : stroff,
			   PP32_CTRL_OPT_STOP_ON_BREAKPOINT(g_cur_vrx218_dev->membase, pp32) ? stron : stroff);
		seq_printf(seq, "breakpoint:\n");
		seq_printf(seq, "     pc0: 0x%08x, %s\n",
			   *PP32_BRK_PC(g_cur_vrx218_dev->membase, pp32, 0),
			   PP32_BRK_GRPi_PCn(g_cur_vrx218_dev->membase, pp32, 0, 0) ? "group 0" : "off");
		seq_printf(seq, "     pc1: 0x%08x, %s\n", *PP32_BRK_PC(g_cur_vrx218_dev->membase, pp32, 1),
			   PP32_BRK_GRPi_PCn(g_cur_vrx218_dev->membase, pp32, 1, 1) ? "group 1" : "off");
		seq_printf(seq, "  daddr0: 0x%08x, %s\n",
			   *PP32_BRK_DATA_ADDR(g_cur_vrx218_dev->membase, pp32, 0),
			   PP32_BRK_GRPi_DATA_ADDRn(g_cur_vrx218_dev->membase, pp32, 0, 0) ? "group 0" : "off");
		seq_printf(seq, "  daddr1: 0x%08x, %s\n",
			   *PP32_BRK_DATA_ADDR(g_cur_vrx218_dev->membase, pp32, 1),
			   PP32_BRK_GRPi_DATA_ADDRn(g_cur_vrx218_dev->membase, pp32, 1, 1) ? "group 1" : "off");
		seq_printf(seq, "  rdval0: 0x%08x\n",
			   *PP32_BRK_DATA_VALUE_RD(g_cur_vrx218_dev->membase, pp32, 0));
		seq_printf(seq, "  rdval1: 0x%08x\n", *PP32_BRK_DATA_VALUE_RD(g_cur_vrx218_dev->membase, pp32, 1));
		seq_printf(seq, "  wrval0: 0x%08x\n", *PP32_BRK_DATA_VALUE_WR(g_cur_vrx218_dev->membase, pp32, 0));
		seq_printf(seq, "  wrval1: 0x%08x\n", *PP32_BRK_DATA_VALUE_WR(g_cur_vrx218_dev->membase, pp32, 1));
	}

	return len;
}

static ssize_t proc_write_pp32(struct file *file, const char __user * buf,
			       size_t count, loff_t * data)
{
	char str[1000];
	char *p;
	...
	if (strcasecmp(p, "restart") == 0)
		*PP32_FREEZE(g_cur_vrx218_dev->membase) &= ~(1 << (pp32 << 4));
	else if (strcasecmp(p, "freeze") == 0)
		*PP32_FREEZE(g_cur_vrx218_dev->membase) |= 1 << (pp32 << 4);
	else if (strcasecmp(p, "start") == 0)
		*PP32_CTRL_CMD(g_cur_vrx218_dev->membase, pp32) =
		...
		*PP32_CTRL_CMD(g_cur_vrx218_dev->membase, pp32) =
		...
		if (strcasecmp(p, "off") == 0) {
			*PP32_BRK_TRIG(g_cur_vrx218_dev->membase, pp32) = PP32_BRK_GRPi_PCn_OFF(0, 0);
			*PP32_BRK_PC_MASK(g_cur_vrx218_dev->membase, pp32, 0) = PP32_BRK_CONTEXT_MASK_EN;
			*PP32_BRK_PC(g_cur_vrx218_dev->membase, pp32, 0) = 0;
		} else {
			addr = get_number(&p, &rlen, 1);
			*PP32_BRK_PC(g_cur_vrx218_dev->membase, pp32, 0) = addr;
			*PP32_BRK_PC_MASK(g_cur_vrx218_dev->membase, pp32, 0) =
			    PP32_BRK_CONTEXT_MASK_EN | PP32_BRK_CONTEXT_MASK(0)
			    | PP32_BRK_CONTEXT_MASK(1) |
			    PP32_BRK_CONTEXT_MASK(2) | PP32_BRK_CONTEXT_MASK(3);
			*PP32_BRK_TRIG(g_cur_vrx218_dev->membase, pp32) =
		...
		if (strcasecmp(p, "off") == 0) {
			...
			*PP32_BRK_PC_MASK(g_cur_vrx218_dev->membase, pp32, 1) =
			    PP32_BRK_CONTEXT_MASK_EN;
			*PP32_BRK_PC(g_cur_vrx218_dev->membase, pp32, 1) = 0;
		} else {
			addr = get_number(&p, &rlen, 1);
			*PP32_BRK_PC(g_cur_vrx218_dev->membase, pp32, 1) = addr;
			*PP32_BRK_PC_MASK(g_cur_vrx218_dev->membase, pp32, 1) =
			    PP32_BRK_CONTEXT_MASK_EN | PP32_BRK_CONTEXT_MASK(0)
			    | PP32_BRK_CONTEXT_MASK(1) |
			    PP32_BRK_CONTEXT_MASK(2) | PP32_BRK_CONTEXT_MASK(3);
		...
		if (strcasecmp(p, "off") == 0) {
			...
			*PP32_BRK_DATA_ADDR_MASK(g_cur_vrx218_dev->membase,
						 pp32, 0) =
			    PP32_BRK_CONTEXT_MASK_EN;
			*PP32_BRK_DATA_ADDR(g_cur_vrx218_dev->membase, pp32,
					    0) = 0;
		} else {
			addr = get_number(&p, &rlen, 1);
			*PP32_BRK_DATA_ADDR(g_cur_vrx218_dev->membase, pp32,
		...
		printk("echo \"<command>\" > /proc/eth/vrx320/pp32\n");
		printk("  command:\n");
		...
		printk
		    ("    daddr0 - daddr0 <addr>/off, set break point data address 0\n");
		printk
		    ("    daddr0 - daddr1 <addr>/off, set break point data address 1\n");
		printk("    help   - print this screen\n");
	}

	if (*PP32_BRK_TRIG(g_cur_vrx218_dev->membase, pp32))
		*PP32_CTRL_OPT(g_cur_vrx218_dev->membase, pp32) =
...
{
	int len = 0;
	u32 base = (unsigned int)g_cur_vrx218_dev->membase;
	...
	for (i = 0; i < 2; i++) {
		if (i != 0)
			seq_printf(seq, ", ");
		seq_printf(seq, "%10u", *(wrx_bc_overdrop_cnt[i]) >= last_wrx_bc_overdrop_cnt[idx][i]
			   ? *(wrx_bc_overdrop_cnt[i]) - last_wrx_bc_overdrop_cnt[idx][i] : *(wrx_bc_overdrop_cnt[i]) + ((unsigned int)(-1) - last_wrx_bc_overdrop_cnt [idx][i]));
	}
	seq_printf(seq, "\n");
	...
	for (i = 0; i < 4; i++) {
		if (i != 0)
			seq_printf(seq, ", ");
		seq_printf(seq, "%10u",
			   *(wrx_crc_err_pdu[i]) >= last_wrx_crc_error_total_pdu[idx][i] ? *(wrx_crc_err_pdu[i]) - last_wrx_crc_error_total_pdu[idx][i]
	...
	for (i = 0; i < 4; i++) {
		if (i != 0)
			seq_printf(seq, ", ");
		seq_printf(seq, "%10u",
			   WAN_RX_MIB_TABLE(i, base)->wrx_total_bytes);
	}
	seq_printf(seq, "\n");

	seq_printf(seq, "TX (Bearer Channels[0-1]):\n");
	seq_printf(seq, "  total_tx_cw:     %10u, %10u\n", *TRANSMIT_CELL_CNT((0), base), *TRANSMIT_CELL_CNT((1), base));
	...
	for (i = 0; i < 4; i++) {
		if (i != 0)
			seq_printf(seq, ", ");
		seq_printf(seq, "%10u",
			   WAN_TX_GIF_CFG(i, base)->deq_pkt_cnt >= last_wtx_total_pdu[idx][i] ? WAN_TX_GIF_CFG(i, base)->deq_pkt_cnt - last_wtx_total_pdu[idx][i]
...
static ssize_t proc_write_wanmib(struct file *file, const char __user * buf,
				 size_t count, loff_t * data)
{
	char str[128];
	char *p;
	...
	if (strcasecmp(p, "clear") == 0 || strcasecmp(p, "clean") == 0) {
		for (i = 0; i < 4; i++) {
			...
			memset((void *)WAN_RX_MIB_TABLE(i, base), 0,
			       sizeof(*WAN_RX_MIB_TABLE(i, base)));
		}
		for (i = 0; i < VRX218_QOSQ_NUM; i++)
			memset((void *)WAN_QOS_MIB_TABLE(i, base), 0,
			       sizeof(*WAN_QOS_MIB_TABLE(i, base)));
	}
...
static int proc_read_dev(struct seq_file *seq, void *v)
{
	...
	for (i = 0;
	     i < ARRAY_SIZE(g_vrx218_dev)
	     && g_cur_vrx218_dev != &g_vrx218_dev[i]; i++) ;
...
static ssize_t proc_write_dev(struct file *file, const char __user * buf,
			      size_t count, loff_t * data)
{
	char str[1024];
	char *p;
	...
	if (strcasecmp(p, "VRX318_US_BONDING_MASTER") == 0)
		...
		for (i = 0;
		     i < ARRAY_SIZE(g_vrx218_idx)
		     && g_vrx218_idx[i] != (int)(*p - '0'); i++) ;
		...
		printk
		    ("echo <VRX318_US_BONDING_MASTER | VRX318_DS_BONDING_MASTER | PCIe Device No.> > /proc/eth/vrx320/dev\n");
...
{
	int len = 0;
	unsigned int base = (unsigned int)g_cur_vrx218_dev->membase;
	volatile bond_conf_t *bconf =
	    (volatile bond_conf_t *)SOC_ACCESS_VRX218_SB(__BOND_CONF, base);
	volatile us_bg_qmap_t *us_bg_qmap =
	    (volatile us_bg_qmap_t *)SOC_ACCESS_VRX218_SB(__US_BG_QMAP, base);
	volatile us_bg_gmap_t *us_bg_gmap =
	    (volatile us_bg_gmap_t *)SOC_ACCESS_VRX218_SB(__US_BG_GMAP, base);
	volatile ds_bg_gmap_t *ds_bg_gmap =
	    (volatile ds_bg_gmap_t *)SOC_ACCESS_VRX218_SB(__DS_BG_GMAP, base);

	seq_printf(seq, "Bonding Config:\n");

	seq_printf(seq,
		   "  BOND_CONF [0x2008]       = 0x%08x, max_frag_size=%u, pctrl_cnt= %u, dplus_fp_fcs_en=%u\n"
		   "                                     bg_num=%u, bond_mode=%u (%s)\n",
		   *(volatile u32 *)bconf, (u32) bconf->max_frag_size,
		   (u32) bconf->polling_ctrl_cnt, (u32) bconf->dplus_fp_fcs_en,
		   (u32) bconf->bg_num, (u32) bconf->bond_mode,
		   (u32) bconf->bond_mode ? "L2 Trunking" : "L1 TC");
	seq_printf(seq,
		   "                           =             e1_bond_en=%u, d5_acc_dis=%u, d5_b1_en=%u\n",
		   (u32) bconf->e1_bond_en, (u32) bconf->d5_acc_dis,
		   (u32) bconf->d5_b1_en);

	seq_printf(seq,
		   "  US_BG_QMAP[0x2009]       = 0x%08x, qmap0=0x%02x, qmap1=0x%02x, qmap2=0x%02x, qmap3=0x%02x\n",
		   *(volatile u32 *)us_bg_qmap, us_bg_qmap->queue_map0,
		   us_bg_qmap->queue_map1, us_bg_qmap->queue_map2,
		   us_bg_qmap->queue_map3);

	seq_printf(seq,
		   "  US_BG_GMAP[0x200A]       = 0x%08x, gmap0=0x%02x, gmap1=0x%02x, gmap2=0x%02x, qmap3=0x%02x\n",
		   *(volatile u32 *)us_bg_gmap, us_bg_gmap->gif_map0,
		   us_bg_gmap->gif_map1, us_bg_gmap->gif_map2,
		   us_bg_gmap->gif_map3);

	seq_printf(seq,
		   "  DS_BG_GMAP[0x200B]       = 0x%08x, gmap0=0x%02x, gmap1=0x%02x, gmap2=0x%02x, qmap3=0x%02x\n",
		   *(volatile u32 *)ds_bg_gmap, ds_bg_gmap->gif_map0,
		   ds_bg_gmap->gif_map1, ds_bg_gmap->gif_map2,
		   ds_bg_gmap->gif_map3);

	seq_printf(seq,
		   "Cross Pci Debug Info --- Read                  Write \n");
	seq_printf(seq, "    min clock cycle:     %-20u, %u\n",
		   *SOC_ACCESS_VRX218_SB(__READ_MIN_CYCLES, base),
		   *SOC_ACCESS_VRX218_SB(__WRITE_MIN_CYCLES, base));
	seq_printf(seq, "    max clock cycle:     %-20u, %u\n",
		   *SOC_ACCESS_VRX218_SB(__READ_MAX_CYCLES, base),
		   *SOC_ACCESS_VRX218_SB(__WRITE_MAX_CYCLES, base));
	seq_printf(seq, "   total access num:     %-20u, %u\n",
		   *SOC_ACCESS_VRX218_SB(__READ_NUM, base),
		   *SOC_ACCESS_VRX218_SB(__WRITE_NUM, base));
	seq_printf(seq, "     total_cycle_lo:     %-20u, %u\n",
		   *SOC_ACCESS_VRX218_SB(__TOTAL_READ_CYCLES_LO, base),
		   *SOC_ACCESS_VRX218_SB(__TOTAL_WRITE_CYCLES_LO, base));
	seq_printf(seq, " *total_lo/acc_num*:     %-20u, %u\n",
		   *SOC_ACCESS_VRX218_SB(__READ_NUM,
					 base) ?
		   *SOC_ACCESS_VRX218_SB(__TOTAL_READ_CYCLES_LO, base) / *SOC_ACCESS_VRX218_SB(__READ_NUM, base) : 0,
		   *SOC_ACCESS_VRX218_SB(__WRITE_NUM, base) ?
		   *SOC_ACCESS_VRX218_SB(__TOTAL_WRITE_CYCLES_LO, base) /
		   *SOC_ACCESS_VRX218_SB(__WRITE_NUM, base) : 0);
	seq_printf(seq, "  total cycle_hi_lo:     0x%08x%08x,   0x%08x%08x\n",
		   *SOC_ACCESS_VRX218_SB(__TOTAL_READ_CYCLES_HI, base),
		   *SOC_ACCESS_VRX218_SB(__TOTAL_READ_CYCLES_LO, base),
		   *SOC_ACCESS_VRX218_SB(__TOTAL_WRITE_CYCLES_HI, base),
		   *SOC_ACCESS_VRX218_SB(__TOTAL_WRITE_CYCLES_LO, base));
...
static int proc_read_bndmib(struct seq_file *seq, void *v)
{
	...
	for (i = 0; i < VRX218_GIF_NUM; i++) {
		seq_printf(seq, "   Gif[%d]:   %-u\n",
			   i, *US_BOND_GIF_MIB(i, g_us_vrx218_dev->membase));
	}

	base = (unsigned int)g_ds_vrx218_dev->membase;
	for (i = 0; i < VRX218_GIF_NUM; i++) {
		...
		seq_printf(seq, "       Total_rx_frag_cnt:      %10u      Total_rx_byte_cnt: %10u    Overflow_frag_cnt: %10u  Overflow_byte_cnt: %10u\n",
			   DS_BOND_GIF_MIB(i, base)->total_rx_frag_cnt,
			   DS_BOND_GIF_MIB(i, base)->total_rx_byte_cnt,
			   DS_BOND_GIF_MIB(i, base)->overflow_frag_cnt,
			   DS_BOND_GIF_MIB(i, base)->overflow_byte_cnt);
		seq_printf(seq, "       Out_of_range_frag_cnt:  %10u      Missing_frag_cnt:  %10u    Timeout_frag_cnt:  %10u\n\n",
			   DS_BOND_GIF_MIB(i, base)->out_of_range_frag_cnt,
			   DS_BOND_GIF_MIB(i, base)->missing_frag_cnt,
	...
	for (i = 0; i < 4; i++) {
		...
		seq_printf(seq, "     conform_pkt_cnt:   %10u    conform_frag_cnt:  %10u   conform_byte_cnt: %10u   no_sop_pkt_cnt:    %10u   no_sop_frag_cnt:   %10u\n",
			   DS_BG_MIB(i, base)->conform_pkt_cnt, DS_BG_MIB(i, base)-> conform_frag_cnt, DS_BG_MIB(i,
						       base)->conform_byte_cnt, DS_BG_MIB(i, base)->no_sop_pkt_cnt, DS_BG_MIB(i, base)-> no_sop_frag_cnt);
...
static ssize_t proc_write_bndmib(struct file *file, const char __user * buf,
				 size_t count, loff_t * data)
{
	char str[128];
	char *p;
	...
	if (strcasecmp(p, "clear") == 0 || strcasecmp(p, "clean") == 0) {
		for (i = 0; i < VRX218_GIF_NUM; i++) {
			ptr = (void *)US_BOND_GIF_MIB(i, g_us_vrx218_dev->membase);
			memset(ptr, 0, sizeof(unsigned int));

			ptr = (void *)DS_BOND_GIF_MIB(i, g_ds_vrx218_dev->membase);
			memset(ptr, 0, sizeof(ds_bond_gif_mib_t));

			ptr = (void *)DS_BG_MIB(i, g_ds_vrx218_dev->membase);
			memset(ptr, 0, sizeof(ds_bg_mib_t));
		}
	} else {
		printk("echo [clear|clean] > /proc/eth/vrx320/bondmib\n");
	}
...
static void __init init_host_desc(void)
{
	...
	p = (volatile unsigned long *)g_host_desc_base.us_fastpath_des_base_virt;
	for (i = 0; i < SOC_US_FASTPATH_DES_NUM; i++) {
		*p = (*p & ~0xF0000000) | 0x80000000;
		p += 2;
	}

	p = (volatile unsigned long *)g_host_desc_base.us_cpupath_des_base_virt;
	for (i = 0; i < SOC_US_CPUPATH_DES_NUM; i++) {
		*p = 0x30000000;
		p += 2;
	}

	p = (volatile unsigned long *)g_host_desc_base.ds_des_base_virt;
...
#define SOC_DS_DES_NUM              64	//  max: 32 (ATM) / 64 (PTM)
...
#define SOC_DS_OAM_DES_NUM_MAX      32
	...
	if (base->us_fastpath_des_num > 0) {
		base->us_fastpath_des_base_virt = (unsigned long)pci_alloc_consistent(NULL,
							SOC_US_FASTPATH_DES_NUM_MAX * 8,
							(dma_addr_t *) & base->us_fastpath_des_base);
		if (base->us_fastpath_des_base_virt == 0) {
			printk("%s failed to allocate us_fastpath_des descriptor base\n", __func__);
			return -ENOMEM;
		}
	}

	if (base->us_cpupath_des_num > 0) {
		base->us_cpupath_des_base_virt = (unsigned long)pci_alloc_consistent(NULL,
							SOC_US_CPUPATH_DES_NUM_MAX * 8,
							(dma_addr_t *) & base->us_cpupath_des_base);
		if (base->us_cpupath_des_base_virt == 0) {
			printk("%s failed to allocate us_cpupath_des descriptor\n", __func__);
			pci_free_consistent(NULL, SOC_US_FASTPATH_DES_NUM_MAX * 8, (void *)base->us_fastpath_des_base_virt,
					    base->us_fastpath_des_base);
			return -ENOMEM;
		}
	}

	if (base->ds_des_num > 0) {
		base->ds_des_base_virt =
		    (unsigned long)pci_alloc_consistent(NULL, SOC_DS_DES_NUM_MAX * 8, (dma_addr_t *) & base-> ds_des_base);
		if (base->ds_des_base_virt == 0) {
			printk("%s failed to allocate ds_des_base descriptor\n", __func__);
			pci_free_consistent(NULL, SOC_US_FASTPATH_DES_NUM_MAX * 8, (void *)base->us_fastpath_des_base_virt,
					    base->us_fastpath_des_base);
			pci_free_consistent(NULL,
					    SOC_US_CPUPATH_DES_NUM_MAX * 8,
					    (void *)base->us_cpupath_des_base_virt,
					    base->us_cpupath_des_base);
			return -ENOMEM;
		}
	}

	if (base->ds_oam_des_num > 0) {
		base->ds_oam_des_base_virt = (unsigned long)pci_alloc_consistent(NULL,
							SOC_DS_OAM_DES_NUM_MAX * 8,
							(dma_addr_t *) & base->ds_oam_des_base);
		if (base->ds_oam_des_base_virt == 0) {
			printk("%s failed to allocate ds_oam_des_base descriptor\n", __func__);
			pci_free_consistent(NULL, SOC_US_FASTPATH_DES_NUM_MAX * 8,
					    (void *)base->us_fastpath_des_base_virt,
					    base->us_fastpath_des_base);
			pci_free_consistent(NULL, SOC_US_CPUPATH_DES_NUM_MAX * 8,
					    (void *)base->us_cpupath_des_base_virt,
					    base->us_cpupath_des_base);
			pci_free_consistent(NULL, SOC_DS_DES_NUM_MAX * 8,
					    (void *)base->ds_des_base_virt,
					    base->ds_des_base);
			return -ENOMEM;
		}
	}
	printk("base->us_fastpath_des_base=%#x\n",
	       (unsigned int)base->us_fastpath_des_base);
	printk("base->us_fastpath_des_num=%#x\n", base->us_fastpath_des_num);
	printk("base->us_cpupath_des_base=%#x\n",
	       (unsigned int)base->us_cpupath_des_base);
	printk("base->us_cpupath_des_num=%#x\n", base->us_cpupath_des_num);
	printk("base->ds_des_base=%#x\n", (unsigned int)base->ds_des_base);
	printk("base->ds_des_num=%#x\n", base->ds_des_num);
	printk("base->ds_oam_des_base=%#x\n",
	       (unsigned int)base->ds_oam_des_base);
	printk("base->ds_oam_des_num=%#x\n", base->ds_oam_des_num);
...
static int __init ppe_init(void)
{
	...
	extern int32_t vrx218_qos_ppe_generic_hook(PPA_GENERIC_HOOK_CMD cmd,
						   void *buffer, uint32_t flag);
#ifdef LANTIQ_ENV
	extern int32_t register_ppe_ext_generic_hook(ppe_generic_hook_t
						     datapath_hook,
						     ppe_generic_hook_t
						     hal_hook);
#endif
	...
	for (i = 0, valid_dev_num = 0; i < dev_num; i++) {
		if (ltq_pcie_ep_dev_info_req(i, IFX_PCIE_EP_INT_PPE, &g_vrx218_dev[valid_dev_num])) {
	...
	if (valid_dev_num == 1) {
		...
		if (g_bonding_sync_buf_page == 0) {
			for (i = 0; i < ARRAY_SIZE(g_vrx218_idx); i++)
				if (g_vrx218_idx[i] >= 0)
					ltq_pcie_ep_dev_info_release (g_vrx218_idx[i]);
			return -ENOMEM;
		}
	...
	ppa_drv_hal_generic_hook = vrx218_qos_ppe_generic_hook;
	proc_file_create();
...
static void __exit ppe_exit(void)
{
	...
	int i;
	struct host_desc_mem *base = &g_host_desc_base;
	...
	if (base->us_fastpath_des_base_virt)
		pci_free_consistent(NULL, SOC_US_FASTPATH_DES_NUM_MAX * 8,
				    (void *)base->us_fastpath_des_base_virt,
				    base->us_fastpath_des_base);
	if (base->us_cpupath_des_base_virt)
		pci_free_consistent(NULL, SOC_US_CPUPATH_DES_NUM_MAX * 8,
				    (void *)base->us_cpupath_des_base_virt,
				    base->us_cpupath_des_base);
	if (base->ds_des_base_virt)
		pci_free_consistent(NULL, SOC_DS_DES_NUM_MAX * 8,
				    (void *)base->ds_des_base_virt,
				    base->ds_des_base);
	if (base->ds_oam_des_base_virt)
		pci_free_consistent(NULL, SOC_DS_OAM_DES_NUM_MAX * 8,
				    (void *)base->ds_oam_des_base_virt,
				    base->ds_oam_des_base);
...
static int __init queue_gamma_map_setup(char *line)
{
	...
	for (i = 0, p = line; i < NUM_ENTITY(queue_gamma_map) && isxdigit(*p);
	     i++) {
