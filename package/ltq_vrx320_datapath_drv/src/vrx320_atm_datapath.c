...
//extern unsigned int normalpoll_start();
//extern unsigned int normalpoll_stop();
//extern unsigned int hrpoll_start();
//extern unsigned int hrpoll_stop();
//#endif
...
#ifdef AKRONITE_BOARD
#define CPU_TO_WAN_TX_DESC_BASE                 ((volatile struct tx_descriptor *)KSEG1ADDR(SOC_US_CPUPATH_DES_BASE_VIRT))
//Re-used US_FASTPATH_DES_BASE for OAM US as CPU_TO_WAN_OAM_TX_DESC_BASE
#define CPU_TO_WAN_OAM_TX_DESC_BASE		((volatile struct tx_descriptor *)KSEG1ADDR(SOC_US_FASTPATH_DES_BASE_VIRT))
#else
...
#define WRX_QUEUE_CONFIG(i)                     ((volatile wrx_queue_config_t *)SOC_ACCESS_VRX218_SB(__WRX_QUEUE_CONFIG + (i) * 10, g_atm_priv_data.vrx218_dev.membase)) /* i < 16 */
#define WTX_QUEUE_CONFIG(i)                     ((volatile wtx_queue_config_t *)SOC_ACCESS_VRX218_SB(__WTX_QUEUE_CONFIG + (i) * 25, g_atm_priv_data.vrx218_dev.membase)) /* i < 16 */

#define HTU_ENTRY(i)                            ((volatile struct htu_entry *)  SOC_ACCESS_VRX218_SB(__HTU_ENTRY_TABLE + (i),  g_atm_priv_data.vrx218_dev.membase)) /*  i < 24  */
#define HTU_MASK(i)                             ((volatile struct htu_mask *)   SOC_ACCESS_VRX218_SB(__HTU_MASK_TABLE + (i),   g_atm_priv_data.vrx218_dev.membase)) /*  i < 24  */
#define HTU_RESULT(i)                           ((volatile struct htu_result *) SOC_ACCESS_VRX218_SB(__HTU_RESULT_TABLE + (i), g_atm_priv_data.vrx218_dev.membase)) /*  i < 24  */

#define DSL_WAN_MIB_TABLE                       ((volatile struct dsl_wan_mib_table *)SOC_ACCESS_VRX218_SB(0x4EF0, g_atm_priv_data.vrx218_dev.membase))

/*
 *  Mailbox IGU0 Registers
 */
#define MBOX_IGU0_ISRS                          SOC_ACCESS_VRX218_ADDR(PPE_REG_ADDR(0x0200), g_atm_priv_data.vrx218_dev.membase)
#define MBOX_IGU0_ISRC                          SOC_ACCESS_VRX218_ADDR(PPE_REG_ADDR(0x0201), g_atm_priv_data.vrx218_dev.membase)
#define MBOX_IGU0_ISR                           SOC_ACCESS_VRX218_ADDR(PPE_REG_ADDR(0x0202), g_atm_priv_data.vrx218_dev.membase)
#define MBOX_IGU0_IER                           SOC_ACCESS_VRX218_ADDR(PPE_REG_ADDR(0x0203), g_atm_priv_data.vrx218_dev.membase)

/*
 *  Mailbox IGU1 Registers
 */
#define MBOX_IGU1_ISRS                          SOC_ACCESS_VRX218_ADDR(PPE_REG_ADDR(0x0204), g_atm_priv_data.vrx218_dev.membase)
#define MBOX_IGU1_ISRC                          SOC_ACCESS_VRX218_ADDR(PPE_REG_ADDR(0x0205), g_atm_priv_data.vrx218_dev.membase)
#define MBOX_IGU1_ISR                           SOC_ACCESS_VRX218_ADDR(PPE_REG_ADDR(0x0206), g_atm_priv_data.vrx218_dev.membase)
#define MBOX_IGU1_IER                           SOC_ACCESS_VRX218_ADDR(PPE_REG_ADDR(0x0207), g_atm_priv_data.vrx218_dev.membase)

#define QSB_CONF_REG(addr)                      SOC_ACCESS_VRX218_ADDR(QSB_CONF_REG_ADDR(addr), g_atm_priv_data.vrx218_dev.membase)
 ...
 *  CGU Register
 */
#define CGU_CLKFSR                              SOC_ACCESS_VRX218_ADDR(0x1E003010, g_atm_priv_data.vrx218_dev.membase)
...
struct atm_priv_data {
    ltq_pcie_ep_dev_t               vrx218_dev;             /*  end point device                        */
...
#endif
struct sk_buff *alloc_skb_tx_aligned(struct sk_buff *, int);
...
#ifdef AKRONITE_BOARD
static int rx_swap_data(unsigned char *data, int len) 
{
	int i = 0;
...
static int swap_data(unsigned char *data, int len) 
{
	int i = 0;
	unsigned int *d;
...
extern void ifx_push_oam(struct atm_vcc *, unsigned char *);
#else
static inline void ifx_push_oam(struct atm_vcc *vcc, unsigned char *cell)
{
...
static DECLARE_TASKLET(g_data_tasklet, do_data_tasklet, 0);
static int g_oam_desc_pos = 0;
static int g_data_desc_pos = 0;

static DEFINE_SPINLOCK(g_mailbox_lock);

static int g_cpu_to_wan_tx_desc_pos = 0;
static int g_cpu_to_wan_oam_tx_desc_pos = 0; //For CPU_TO_WAN_OAM_TX_DESC_BASE
static DEFINE_SPINLOCK(g_cpu_to_wan_tx_desc_lock);
static DEFINE_SPINLOCK(g_cpu_to_wan_oam_tx_desc_lock); //For CPU_TO_WAN_OAM_TX_DESC_BASE
...
static int ppe_send(struct atm_vcc *vcc, struct sk_buff *skb)
{
	...
	if ( skb->len > aal5s_max_packet_size ) {
		printk ("skb->len > aal5s_max_packet_size : skb->len: %d\n", skb->len);
		return -EOVERFLOW;
	}
	...
	/*  update descriptor   */
	//reg_desc.dataptr	= dma_map_single(NULL, (void *)dma_to_virt(NULL, reg_desc.dataptr), reg_desc.datalen, DMA_TO_DEVICE);
	dma_sync_single_for_device (NULL, reg_desc.dataptr, reg_desc.datalen, DMA_TO_DEVICE);
	reg_desc.byteoff    = 0; //byteoff;
	//reg_desc.datalen    = skb->len; //This we already filled.
	reg_desc.mpoa_pt    = 1;
	...
	UPDATE_VCC_STAT(conn, tx_bytes, reg_desc.datalen);
...
#if defined (CONFIG_MIPS) && !(CONFIG_MIPS_UNCACHED)
    /*  write back to physical memory   */
    dma_cache_wback((unsigned int)skb->data, skb->len);
#endif

    /*  allocate descriptor */
    spin_lock_irqsave(&g_cpu_to_wan_oam_tx_desc_lock, sys_flag);
    //desc = CPU_TO_WAN_TX_DESC_BASE + g_cpu_to_wan_tx_desc_pos;
    desc = CPU_TO_WAN_OAM_TX_DESC_BASE + g_cpu_to_wan_oam_tx_desc_pos; //We re-use US_FASTPATH_DESC for OAM US.
    //if (  desc->own )    //  PPE hold
    if ( ! desc->own )    //  PPE hold
    {
        spin_unlock_irqrestore(&g_cpu_to_wan_oam_tx_desc_lock, sys_flag);
        err("OAM not alloc tx connection");
	printk("OAM not find queue\n");
        return -EIO;
    }
    if ( ++g_cpu_to_wan_oam_tx_desc_pos == CPU_TO_WAN_TX_DESC_NUM )
        g_cpu_to_wan_oam_tx_desc_pos = 0;
//	if ( ++g_cpu_to_wan_tx_desc_pos == CPU_TO_WAN_TX_DESC_NUM )
//		g_cpu_to_wan_tx_desc_pos = 0;
    spin_unlock_irqrestore(&g_cpu_to_wan_oam_tx_desc_lock, sys_flag);
    r = swap_data(cell + 4, CELL_SIZE);
    if (r) {
	desc->datalen = CELL_SIZE + r;
    }else{
	desc->datalen = CELL_SIZE;
    }
    memcpy (dma_to_virt(NULL, desc->dataptr), cell, desc->datalen);
    /*  load descriptor from memory */
    reg_desc = *desc;
...
#endif
    reg_desc.byteoff    = 0; //(unsigned int)skb->data & (DMA_ALIGNMENT - 1);
    ...
    //reg_desc.own        = 1; //If we use US_CPU_DES
    reg_desc.own        = 0; //Changed this since we re-use US_FASTPATH_DESC
    reg_desc.c          = 0;
    reg_desc.sop = reg_desc.eop = 1;
	//printk("%p %p\n",reg_desc.dataptr,desc);
...
static void atm_encapsulate_frame(struct sk_buff *skb, struct flag_header *header, int conn)
{
	...
	//printk("In WRX_QUEUE_CONFIG(conn)->mpoa_mode\n");
...
#ifdef AKRONITE_BOARD
	// LITTLE ENDIAN
            ((u32 *)skb->data)[0] = 0x0003AAAA;
            ((u32 *)skb->data)[1] = 0x0700C280;
#else
	...
	// LITTLE ENDIAN
        ...
        case 2: //  PPPoA
#ifdef AKRONITE_BOARD
	    switch ( (proto_type = *(u16 *)(skb->data + 12)))
	    {
	    case 0x0008: proto_type = 0x2100; break;
	    case 0xDD86: proto_type = 0x5700; break;
#else
	...
	//printk("Else part of WRX_QUEUE_CONFIG(conn)->mpoa_mode\n");
...
#ifdef AKRONITE_BOARD 
int atm_push(struct sk_buff *skb, struct flag_header *header, unsigned int ifid)
#else
...
/** Timer functions for RX **/
int	g_timer_rx_poll = 0;
int	g_poll_interval_ms = 700;
...
unsigned int vrx320_hrpoll_stop(void)
{
	...
	//return 0;
	if(timer_started==1) {
		//printk("##################################In polling - Stopping timer#####################################\n");
  	ret = hrtimer_cancel(&hr_timer);
	}
	//ret = hrtimer_try_to_cancel(&hr_timer);
	if(ret<0){
	  printk("cancelling hr timer failed\n");
	}
	timer_started = 0;
	return 0;
}
...
{
	ktime_t ktime,now;
	tasklet_schedule (&g_data_tasklet);

	if(g_timer_rx_poll == 1) {
...
static void do_data_tasklet(unsigned long arg)
{
	volatile struct rx_descriptor *tmp, *pdesc;
	unsigned int len;
	struct flag_header *header;
	struct sk_buff *skb;

	/* Check if the data isr set */
	if(!(*MBOX_IGU1_ISR & 0x20)) {
		*MBOX_IGU1_IER |= 0x20; //Set 6th bit
		tmp = (volatile struct rx_descriptor *)g_host_desc_base.ds_des_base_virt;
		pdesc = &tmp[g_data_desc_pos];
		if (pdesc->own) {
			if(++g_rx_empty_cnt > 3) {
				g_timer_rx_poll = 0;
				g_rx_empty_cnt = 0;
			} else g_timer_rx_poll = 1;
	...
	while ( 1 ) {
		...
		if ( pdesc->own ) {
			*MBOX_IGU1_ISRC = 0x20; //Clear 6th bit
			g_status_dbg = *MBOX_IGU1_ISR;
			g_status_dbg = g_status_dbg << 16;
			if ( pdesc->own ) {
				//*MBOX_IGU1_IER |= 0x20; //Set 6th bit
				vrx320_hrpoll_start();
				g_timer_rx_poll = 1;
		...
		//dma_unmap_single(NULL, (dma_addr_t)pdesc->dataptr, len, DMA_FROM_DEVICE);
		dma_sync_single_for_cpu(NULL, pdesc->dataptr, 1600, DMA_FROM_DEVICE);

		//memcpy(skb->data, dma_to_virt(NULL, pdesc->dataptr), len+4);
		vrx320_memcpy_swap(skb->data, (char *)dma_to_virt(NULL, pdesc->dataptr), len + 4);
		...
		if ( skb ) {
			...
			/* header->src_itf should be 6 (ATM IPoA/PPPoA) or 7 (ATM EoA or PTM) */
			atm_push(skb, header, header->src_itf);
		}

		pdesc->own=1; 
		pdesc->data_len=1600;

		if ( ++g_data_desc_pos == SOC_ATM_DS_DES_NUM ) {
			g_data_desc_pos = 0;
		}
...
static void do_oam_tasklet(unsigned long arg)
{
	...
	*MBOX_IGU1_IER |= 0x02;
    ...
    desq_oam_cfg_ctxt = (volatile desq_cfg_ctxt_t *)SOC_ACCESS_VRX218_SB(__DS_OAM_DESQ_CFG_CTXT, g_atm_priv_data.vrx218_dev.membase);
    lclq_oam_cfg_ctxt = (volatile desq_cfg_ctxt_t *)SOC_ACCESS_VRX218_SB(__DS_TC_OAM_LOCAL_Q_CFG_CTXT, g_atm_priv_data.vrx218_dev.membase);
    while ( 1 ) {
#ifdef AKRONITE_BOARD
	tmp = (volatile struct rx_descriptor *)g_host_desc_base.ds_oam_des_base_virt;
	pdesc = &tmp[g_oam_desc_pos];
#else
...
*/
	...
	test = (char *) dma_to_virt(NULL, pdesc->dataptr); //|0x60000000);

	rx_swap_data(test+4, pdesc->data_len);

	header = (struct uni_cell_header *) test;
...
/* Workaround: Force change and write ownership bit in firmware side - Need fix in Firmware XXX */
	//tmp_desc = (struct rx_descriptor *) CPHYSADDR(SOC_ACCESS_VRX218_SB(__DS_OAM_DES_LIST_BASE, g_atm_priv_data.vrx218_dev.membase)) + g_oam_desc_pos;

        //dword_mem_write((void *)tmp_desc, (void *)pdesc, sizeof(struct rx_descriptor));
/* Workaround end */
...
#endif
struct sk_buff *alloc_skb_tx_aligned(struct sk_buff *skb, int len)
{
    if ( skb )
        dev_kfree_skb_any(skb);
...
#else
		/* clean_dcache_area((u32)skb->data - sizeof(u32), sizeof(u32)); */
#endif
        	ATM_SKB(skb)->vcc = NULL;
	}
...
static irqreturn_t mailbox_irq_handler(int irq, void *dev_id)
{
    ...
    *MBOX_IGU1_ISRC = mailbox_signal & (~0x22);
    spin_unlock(&g_mailbox_lock);
    *MBOX_IGU1_IER &= ~0x22; // Disable oam and data interrupt
/******************************************************
 * Remove, Driver should Disable IER bit0 by default
...
********************************************************/
    ...
    //if ( (mailbox_signal & 0x02) ) { Always launch the tasklet, do the isr check in the tasklet
    //    spin_lock(&g_mailbox_lock);
    //    *MBOX_IGU1_IER &= ~0x02;
    //    spin_unlock(&g_mailbox_lock);
        tasklet_schedule(&g_oam_tasklet); 
     ...
     */
/*********************************************************
 * No clear (will clear bit 1 and bit 5 in the tasklet)
    spin_lock(&g_mailbox_lock);
...
ssize_t proc_write_prio(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
    ...
    while ( get_token(&p1, &p2, &len, &colon) )
    {
        ...
        {
            printk("echo <pvc vpi.vci> prio xx queue xx [prio xx queue xx] > /proc/vrx320/prio\n");
            printk("echo pvc vpi.vci <add/del> > /proc/vrx320/prio\n");
            break;
        }
...
ssize_t proc_write_dsl_vlan(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
    ...
    while ( get_token(&p1, &p2, &len, &colon) )
    {
        if ( strcasecmp(p1, "help") == 0 )
        {
            printk("echo <pvc vpi.vci> <vlan xx> [bypass [enable|disbable]] > /proc/vrx320/dsl_vlan\n");
...
ssize_t proc_write_cell(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
    ...
    while ( get_token(&p1, &p2, &len, &colon) ) {
        if ( strcasecmp(p1, "help") == 0 ) {
            printk("echo <pvc vpi.vci> [pti] [gfc] [clp] <hex data> > /proc/vrx320/cell\n");
            conn = -1;
            break;
        }
        else if ( strcasecmp(p1, "pvc") == 0 ) {
...
static int atm_showtime_enter(const unsigned char line_idx,struct port_cell_info *port_cell, void *xdata_addr)
{
    ...
    printk("ATM Line[%d]: DSL entered showtime\n", line_idx);
...
static int atm_showtime_exit(const unsigned char line_idx)
{
    ...
    printk("ATM Line[%d]: DSL left showtime\n", line_idx);
...
int __init vrx218_atm_datapath_init(const ltq_pcie_ep_dev_t *p_vrx218_dev)
{
    int ret;
    int i;
