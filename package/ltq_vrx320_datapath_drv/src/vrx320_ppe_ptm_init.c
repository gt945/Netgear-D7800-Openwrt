...
#include "vrx320_edma.h"
#include "vrx320_ppe_fw_const.h"
#include "vrx320_regs.h"
...
static unsigned long g_sync_buf_base = 0x40000000;	/* AKRONITE FIX */
...
struct sk_buff *__get_skb_pointer(unsigned int dataptr, const char *func_name,
				  unsigned int line_num)
{
	...
	skb_dataptr = (unsigned int)dma_to_virt(NULL, (dataptr - 4));
...
struct sk_buff *alloc_skb_tx(unsigned int len)
{
	struct sk_buff *skb;
	...
	if (skb) {
		...
		if (((u32) skb->data & (DMA_ALIGNMENT - 1)) != 0) {
			panic("%s:%d: unaligned address!", __FUNCTION__,
			      __LINE__);
		}
...
static struct sk_buff *g_skb_tx[640] = { 0 };
static unsigned int g_skb_tx_count = 0;

dma_addr_t alloc_dataptr_skb(void)
{
	struct sk_buff *skb = alloc_skb_tx(1600);

	if (skb) {
		ASSERT(g_skb_tx_count < ARRAY_SIZE(g_skb_tx),
		       "too many skb for tx");
#ifdef LANTIQ_ENV
		put_skb_to_dbg_pool(skb);
#endif
		g_skb_tx[g_skb_tx_count++] = skb;
		return dma_map_single(NULL, skb->data, 1600, DMA_TO_DEVICE);
	} else
		return (dma_addr_t)NULL;
}

static struct sk_buff *g_skb_fragments[576] = { 0 };

static unsigned int g_skb_fragments_count = 0;

dma_addr_t alloc_dataptr_fragment(unsigned int bytes)
{
	struct sk_buff *skb = alloc_skb_tx(bytes);

	if (skb) {
		ASSERT(g_skb_fragments_count < ARRAY_SIZE(g_skb_fragments),
		       "too many skb fragments");
		g_skb_fragments[g_skb_fragments_count++] = skb;
		return dma_map_single(NULL, skb->data, 1600, DMA_FROM_DEVICE);
	} else
		return (dma_addr_t) NULL;
}

void *alloc_dataptr_fragment_notrack(unsigned int bytes)
{
	struct sk_buff *skb = alloc_skb_tx(bytes);
...
static void __exit free_dataptr_fragments(void)
{
	...
	for (i = 0; i < g_skb_fragments_count; i++) {
		if(g_skb_fragments[i]) {
			dma_unmap_single(NULL, virt_to_dma(NULL, (g_skb_fragments[i]->data)), 1600, DMA_FROM_DEVICE);
			dev_kfree_skb_any(g_skb_fragments[i]);
			g_skb_fragments[i] = NULL;
		}
	}

	for (i = 0; i < g_skb_tx_count; i++) {
		if(g_skb_tx[i]) {
			dma_unmap_single(NULL, virt_to_dma(NULL, (g_skb_tx[i]->data)), 1600, DMA_TO_DEVICE);
			dev_kfree_skb_any(g_skb_tx[i]);
			g_skb_tx[i] = NULL;
		}
...
void dword_mem_write(void *__dst_addr, void *__src_addr,
...
void vrx218_tc_reg_init(unsigned long base)
{
	...
	*SOC_ACCESS_VRX218_ADDR(VRX_PPE_SFSM_CFG1, base) = 0x00030070;
	...
	//KEEP IDLE (BC0 & BC1)
	*SOC_ACCESS_VRX218_ADDR(VRX_PPE_SFSM_CFG0, base) =
	    (*SOC_ACCESS_VRX218_ADDR(VRX_PPE_SFSM_CFG0, base) | (1 << 15));
	*SOC_ACCESS_VRX218_ADDR(VRX_PPE_SFSM_CFG1, base) =
	    (*SOC_ACCESS_VRX218_ADDR(VRX_PPE_SFSM_CFG1, base) | (1 << 15));
...
void vrx218_pdma_init(unsigned long base, int is_bonding)
{
	...
	*SOC_ACCESS_VRX218_ADDR(VRX_PPE_PDMA_RX_MAX_LEN_REG, base) =
	    is_bonding ? 0x02040204 : 0x02040604;
...
#define DS_FLOW_CTRL_CFG    0x2026
	*SOC_ACCESS_VRX218_SB(DS_FLOW_CTRL_CFG, base) = 0x3C30;
	...
	dword_mem_clear((unsigned int *)(&qosq_flow_ctrl_cfg),
			sizeof(qosq_flow_ctrl_cfg));
	...
	for (i = 0; i < 2; i++) {
		dst_addr = (void *) SOC_ACCESS_VRX218_SB((__RX_BC0_CFG_STATS_CFG +
					  (i * sizeof(rx_bc_cfg) / sizeof(unsigned int))), base);
	...
	tx_bc_cfg.uflw_wm = 2;
	for (i = 0; i < 2; i++) {
		dst_addr = (void *) SOC_ACCESS_VRX218_SB((__TX_BC0_CFG_STATS_CFG +
					  (i * sizeof(tx_bc_cfg) / sizeof(unsigned int))), base);
	...
	for (i = 0; i < 4; i++) {
		dst_addr = (void *) SOC_ACCESS_VRX218_SB((__RX_GIF0_CFG_STATS_CFG +
					  (i * sizeof(rx_gamma_itf_cfg) / sizeof(unsigned int))), base);
	...
	for (i = 0; i < 4; i++) {
		 ...
		 */
		dst_addr = (void *) SOC_ACCESS_VRX218_SB((__TX_GIF0_CFG_STATS_CFG +
					  (i * sizeof(tx_gamma_itf_cfg) / sizeof(unsigned int))), base);
...
void vrx218_gen_cfg_init(unsigned long base, int is_bonding,
			 unsigned int vrx218_id, unsigned long peer_base)
{
	...
	task_cfg[0].pp32_core_id = 0;
	task_cfg[1].pp32_core_id = 1;
	task_cfg[0].us_bonding_master = 0;
	task_cfg[1].us_bonding_master = 0;
	task_cfg[0].us_segment_en = 0;
	task_cfg[1].us_segment_en = 0;
	task_cfg[0].us_buf_release_en = 0;
	task_cfg[1].us_buf_release_en = 0;

	task_cfg[0].ds_bonding_master = 0;
	task_cfg[1].ds_bonding_master = 0;
	task_cfg[0].ds_pkt_dispatch_en = 0;
	task_cfg[1].ds_pkt_dispatch_en = 0;
	task_cfg[0].ds_pkt_reconstruct_en = 0;
	task_cfg[1].ds_pkt_reconstruct_en = 0;
	task_cfg[0].ds_pkt_flush_en = 0;
	task_cfg[1].ds_pkt_flush_en = 0;

	task_cfg[0].tc_us_en = 1;
	task_cfg[1].tc_us_en = 0;
	task_cfg[0].tc_ds_en = 1;
	task_cfg[1].tc_ds_en = 0;

	task_cfg[0].des_sync_en = 0;
	task_cfg[1].des_sync_en = 1;
	task_cfg[0].edma_write_lle_gen_en = 0;
	task_cfg[1].edma_write_lle_gen_en = 1;
	task_cfg[0].edma_read_lle_gen_en = 0;
	task_cfg[1].edma_read_lle_gen_en = 1;
	task_cfg[0].edma_post_proc_en = 0;
	task_cfg[1].edma_post_proc_en = 1;
	task_cfg[0].qos_wfq_shaping_en = 0;
	task_cfg[1].qos_wfq_shaping_en = 1;
	task_cfg[0].qos_dispatch_en = 0;
	task_cfg[1].qos_dispatch_en = 1;
	task_cfg[0].qos_replenish_en = 0;
	task_cfg[1].qos_replenish_en = 1;	//  enable for QoS

	if (is_bonding == 1) {

		bond_conf_t bond_conf;
		dword_mem_clear((unsigned int *)(&bond_conf),
				sizeof(bond_conf));
...
#if 0
		peer_fw_access_base =
		    (unsigned int)SOC_ACCESS_VRX218_ADDR(0x1E218000, peer_base);
		*SOC_ACCESS_VRX218_SB(__BOND_PEER_SB_BASE, base) =
		    (0x20000000 | peer_fw_access_base) & 0x3FFFFFFF;
#else
	...
	if ((is_bonding == 1) && (vrx218_id == VRX218_US_BONDING_MASTER)) {
		...
		*SOC_ACCESS_VRX218_SB(__US_BG_GMAP, base) = 0x00002211;
		...
		task_cfg[0].us_bonding_des_sync = 0;
		task_cfg[1].us_bonding_des_sync = 1;
		task_cfg[0].ds_bonding_des_sync = 0;
		task_cfg[1].ds_bonding_des_sync = 1;
		...
		task_cfg[0].qos_wfq_shaping_en = 1;
		task_cfg[1].qos_wfq_shaping_en = 0;
		task_cfg[0].qos_dispatch_en = 1;
		task_cfg[1].qos_dispatch_en = 0;
		task_cfg[0].qos_replenish_en = 1;
		task_cfg[1].qos_replenish_en = 0;
		...
		task_cfg[0].des_sync_en = 1;
		task_cfg[1].des_sync_en = 0;
		task_cfg[0].edma_write_lle_gen_en = 0;
		task_cfg[1].edma_write_lle_gen_en = 1;
		task_cfg[0].edma_read_lle_gen_en = 0;
		task_cfg[1].edma_read_lle_gen_en = 1;
...
void vrx218_edma_init(unsigned long base, int lle_in_sb, int cdma_write_data_en)
{
	...
	*SOC_ACCESS_VRX218_SB(0x2514, base) = 10;
	...
	if (cdma_write_data_en == 0) {
		...
		*EDMA_CH_IDX(pcie_ep_cfg_addr) = EDMA_WRITE_CH;
		...
		dword_mem_write((unsigned int *)(EDMA_CH_CTRL(pcie_ep_cfg_addr)),
				(unsigned int *)(&edma_ch_ctrl), sizeof(edma_ch_ctrl));
		...
		*EDMA_LL_PTR_HIGH(pcie_ep_cfg_addr) = 0;
		...
		if (lle_in_sb == 1)
			edma_ch_ctxt.edma_lle_fpi_base =
			    VRX218_ACCESS_VRX218_SB(__DS_EDMA_LLE_BASE);
		else
	...
	*EDMA_LL_PTR_HIGH(pcie_ep_cfg_addr) = 0;
	...
	dst_addr = (void *)((unsigned char *)(dst_addr) +
...
void vrx218_ds_soc_des_init(unsigned long base, int is_bonding,
			    unsigned int vrx218_id)
{
	unsigned int i, *dst_addr;
	rx_descriptor_t rx_descriptor;
	unsigned int ddr_base = 0x20000000;

	dword_mem_clear(&rx_descriptor, sizeof(rx_descriptor));
	dst_addr = (void *)SOC_ACCESS_VRX218_SB((__DDR_BASE), base);
	dword_mem_write(dst_addr, &ddr_base, sizeof(ddr_base));
	...
	if (is_bonding == 0) {
		...
		for (i = 0; i < SOC_DS_DES_NUM; i++) {
			rx_descriptor.dataptr = alloc_dataptr_fragment(1600);
...
#if 0
int vrx218_des_sync_init(unsigned long base, int is_bonding, unsigned int vrx218_id)
{
	unsigned int *dst_addr;
	des_sync_cfg_ctxt_t des_sync_cfg_ctxt;
	...
	if (is_bonding == 0) {
		...
		des_sync_cfg_ctxt.ext_desc_base_addr =
		    0x20000000 + SOC_US_FASTPATH_DES_BASE;

		dst_addr =
		    (void *)SOC_ACCESS_VRX218_SB(__DES_SYNC_CFG_CTXT, base);
		dword_mem_write(dst_addr, (unsigned int *)(&des_sync_cfg_ctxt),
				sizeof(des_sync_cfg_ctxt));

		//CPU-Path Sync Config/Context
		des_sync_cfg_ctxt.sync_type = US_READ_WRITE_SYNC;
		//des_sync_cfg_ctxt.us_des_polling_needed = 0;
		des_sync_cfg_ctxt.max_polling_intv = 15;
		des_sync_cfg_ctxt.desq_cfg_ctxt = __US_CPU_INQ_DES_CFG_CTXT;
		//NOTE: Only for CDMA add 0x20000000
		des_sync_cfg_ctxt.ext_desc_base_addr =
		    0x20000000 + SOC_US_CPUPATH_DES_BASE;

		dst_addr =
		    (void *)SOC_ACCESS_VRX218_SB((__DES_SYNC_CFG_CTXT + 32),
						 base);
		dword_mem_write(dst_addr, (unsigned int *)(&des_sync_cfg_ctxt),
				sizeof(des_sync_cfg_ctxt));
	}
	if (((is_bonding == 1) && (vrx218_id == VRX218_US_BONDING_MASTER))) {
		printk("Polling intv: 15 and CPU path sync off\n");
		//Fast-Path Sync Config/Context
		des_sync_cfg_ctxt.sync_type = US_READ_WRITE_SYNC;
		//des_sync_cfg_ctxt.us_des_polling_needed = 0;
		des_sync_cfg_ctxt.max_polling_intv = 15;
		...
		des_sync_cfg_ctxt.max_polling_intv = 10;
		//des_sync_cfg_ctxt.desq_cfg_ctxt      = __US_CPU_INQ_DES_CFG_CTXT;
		des_sync_cfg_ctxt.desq_cfg_ctxt = 0;
		//NOTE: Only for CDMA add 0x20000000
		des_sync_cfg_ctxt.ext_desc_base_addr =
		    0x20000000 + SOC_US_CPUPATH_DES_BASE;
	...
	if ((is_bonding == 0)
	    || ((is_bonding == 1) && (vrx218_id == VRX218_DS_BONDING_MASTER))) {
		...
		//NOTE: Only for CDMA add 0x20000000
		des_sync_cfg_ctxt.ext_desc_base_addr =
		    0x20000000 + SOC_DS_DES_BASE;

		if ((is_bonding == 1)
		    && (vrx218_id == VRX218_DS_BONDING_MASTER)) {
			dst_addr = (void *)SOC_ACCESS_VRX218_SB(__DES_SYNC_CFG_CTXT, base);
		} else {
			dst_addr = (void *)SOC_ACCESS_VRX218_SB((__DES_SYNC_CFG_CTXT + (2 * 32)), base);
		}
...
void vrx218_us_cpupath_desq_cfg_ctxt_init(unsigned long base)
{
	...
	dword_mem_clear((unsigned int *)(&us_cp_desq_cfg_ctxt),
			sizeof(us_cp_desq_cfg_ctxt));
...
//  is_bonding = 1, vrx218_id = 0: bonding application, us_bonding_master
//  is_bonding = 1, vrx218_id = 1: bonding application, ds_bonding_master
void vrx218_bonding_des_sync_init(unsigned long base, int is_bonding,
				  unsigned int vrx218_id)
{
	unsigned int i, *dst_addr;
	...
	for (i = 0; i < 4; i++) {
		...
		//bond_des_sync_cfg_ctxt.soc_sync_addr = 0x20000000 | virt_to_dma(NULL, SOC_BOND_US_DES_SYNC_BUF_GIF4 + (i * 56 * 4));
		bond_des_sync_cfg_ctxt.soc_sync_addr = 0x20000000 + virt_to_dma(NULL, (void *) SOC_BOND_US_DES_SYNC_BUF_GIF4 +
					     (i * 56 * 4));

		bond_des_sync_cfg_ctxt.cdma_rx_des_dw1 = (VRX218_ACCESS_VRX218_SB(__BOND_US_DES_SYNC_BUF_BASE));
		...
		//*SOC_ACCESS_VRX218_SB(__US_BOND_SOC_SYNC_ADDR_GIF4 + i, base)         = 0x20000000 | virt_to_dma(NULL, SOC_BOND_US_DES_SYNC_BUF_GIF4 + (i * 56 * 4));
		*SOC_ACCESS_VRX218_SB(__US_BOND_SOC_SYNC_ADDR_GIF4 + i, base) =
		    0x20000000 + virt_to_dma(NULL, (void *) SOC_BOND_US_DES_SYNC_BUF_GIF4 + (i * 56 * 4));
		...
		printk("SOC_BOND_US_DES_SYNC_BUF_GIF4=%#x\n",
		       (unsigned int)virt_to_dma(NULL, (void *) SOC_BOND_US_DES_SYNC_BUF_GIF4));
		for (j = 0; j < 128; j++) {
	...
	*SOC_ACCESS_VRX218_SB(__BOND_US_DES_SYNC_RX_DES_DW1, base) = CPHYSADDR(VRX218_ACCESS_VRX218_SB(__BOND_US_DES_SYNC_BUF_BASE));
	...
	//bond_des_sync_cfg_ctxt.soc_sync_addr = 0x20000000 | virt_to_dma(NULL, SOC_BOND_DS_DES_SYNC_BUF);
	bond_des_sync_cfg_ctxt.soc_sync_addr = 0x20000000 + virt_to_dma(NULL, (void *)SOC_BOND_DS_DES_SYNC_BUF);
	...
	//*SOC_ACCESS_VRX218_SB(__DS_BOND_SOC_SYNC_ADDR, base)         = 0x20000000 | virt_to_dma(NULL, SOC_BOND_DS_DES_SYNC_BUF);
	//*SOC_ACCESS_VRX218_SB(__DS_BOND_SOC_SYNC_ENQ_CNT_ADDR, base) = 0x20000000 | virt_to_dma(NULL, SOC_BOND_DS_DES_SYNC_BUF + 0);
	//*SOC_ACCESS_VRX218_SB(__DS_BOND_SOC_SYNC_DEQ_CNT_ADDR, base) = 0x20000000 | virt_to_dma(NULL, SOC_BOND_DS_DES_SYNC_BUF + 4);
	*SOC_ACCESS_VRX218_SB(__DS_BOND_SOC_SYNC_ADDR, base) = 0x20000000 + virt_to_dma(NULL, (void *)SOC_BOND_DS_DES_SYNC_BUF);
	*SOC_ACCESS_VRX218_SB(__DS_BOND_SOC_SYNC_ENQ_CNT_ADDR, base) = 0x20000000 + virt_to_dma(NULL,
				     (void *)SOC_BOND_DS_DES_SYNC_BUF + 0);
	*SOC_ACCESS_VRX218_SB(__DS_BOND_SOC_SYNC_DEQ_CNT_ADDR, base) = 0x20000000 + virt_to_dma(NULL,
				     (void *)SOC_BOND_DS_DES_SYNC_BUF + 4);
...
void vrx218_us_qos_cfg_init(unsigned long base, int is_bonding,
			    unsigned int vrx218_id)
{
	unsigned int i, *dst_addr;
	...
	dword_mem_write(dst_addr, (unsigned int *)(&fp_qos_cfg),
			sizeof(fp_qos_cfg));
	...
	dword_mem_write(dst_addr, (unsigned int *)(&outq_qos_cfg_ctxt),
			sizeof(outq_qos_cfg_ctxt));
	...
	outq_qos_cfg_ctxt.l3_shaping_cfg_ptr = 0;
	outq_qos_cfg_ctxt.l3_shaping_cfg_idx = QOSQ_L3_SHAPER_ID;
	outq_qos_cfg_ctxt.desq_cfg_ctxt +=
...
void vrx218_us_qos_des_init(unsigned long base, int is_bonding,
			    unsigned int vrx218_id)
{
	unsigned int i, *dst_addr;
	tx_descriptor_t tx_descriptor;
	...
	for (i = 0; i < 512; i++) {
		unsigned int dataptr = alloc_dataptr_skb();
		tx_descriptor.dataptr = dataptr;
...
void vrx218_us_bg_ctxt_init(unsigned long base)
{
	...
	for (i = 0; i < 4; i++) {
		...
		dst_addr = (void *) SOC_ACCESS_VRX218_SB((__US_BG_CTXT_BASE +
					  (i * (sizeof(us_bg_ctxt_t) / sizeof(unsigned int)))), base);
...
//  is_bonding = 1, vrx218_id = 0: bonding application, us_bonding_master
//  is_bonding = 1, vrx218_id = 1: bonding application, ds_bonding_master
void vrx218_us_fragq_desq_cfg_ctxt_init(unsigned long base, int is_bonding,
					unsigned int vrx218_id)
{
	...
	for (i = start_fragq_id; i < 8; i++) {
		...
		fragq_desq_cfg_ctxt.des_base_addr =
		    (__US_FRAGQ_DES_LIST_BASE + (i * 48));
		fragq_desq_cfg_ctxt.bp_des_base_addr =
		    (__US_FRAGQ_DES_LIST_BASE + (i * 48) + (16 * 2));

		dst_addr = (void *) SOC_ACCESS_VRX218_SB((__US_FRAGQ_CFG_CTXT_BASE +
					  (i * (sizeof(desq_cfg_ctxt_t) / sizeof(unsigned int)))), base);
		...
		p_tx_descriptor = (tx_descriptor_t *) SOC_ACCESS_VRX218_SB((__US_FRAGQ_DES_LIST_BASE + (i * 48)), base);	//FIXME
		...
		if (vrx218_id == VRX218_DS_BONDING_MASTER) {
			...
			for (j = 0; j < fragq_desq_cfg_ctxt.des_num; j++, p_tx_descriptor++) {
				if (j % 4 == 0) {
					p_tx_descriptor = (tx_descriptor_t *) KSEG1ADDR (SOC_BOND_US_DES_SYNC_BUF_GIF4 +
					     ((i - 4) * (128 * 4)) + ((j / 4) * 64) + 8);
					p_tx_descriptor->own = !fragq_desq_cfg_ctxt.des_in_own_val;
				}
			}
...
void vrx218_edma_copy_ch_init(unsigned long base, int is_bonding,
			      unsigned int vrx218_id, int cdma_write_data_en)
{
	...
	if (is_bonding == 0) {
		...
		dst_addr =
		    (void *)SOC_ACCESS_VRX218_SB(__US_EDMA_COPY_CH_CFG, base);
		dword_mem_write(dst_addr, (unsigned int *)(&copy_ch_cfg),
				sizeof(copy_ch_cfg));
		...
		for (i = 0; i < 1; i++) {
			...
			copy_ch_cfg.dstq_ctxt_ptr +=
			    sizeof(desq_cfg_ctxt_t) / sizeof(unsigned int);
			dst_addr +=
			    sizeof(edma_copy_ch_cfg_t) / sizeof(unsigned int);
			dword_mem_write(dst_addr,
					(unsigned int *)(&copy_ch_cfg),
					sizeof(copy_ch_cfg));
		}
		/*
		...
		for (i = 0; i < 4; i++) {
			...
			local_desq_cfg_ctxt.des_base_addr =
			    __US_TC_LOCAL_Q_DES_LIST_BASE + (des_cnt * 2);
			dst_addr = (void *) SOC_ACCESS_VRX218_SB((__US_TC_LOCAL_Q_CFG_CTXT_BASE
						  + (i * (sizeof(desq_cfg_ctxt_t) / sizeof(unsigned int)))), base);
		...
		if (cdma_write_data_en == 0) {
			...
			dst_addr =
			    (void *)SOC_ACCESS_VRX218_SB(__DS_EDMA_COPY_CH_CFG,
							 base);
			dword_mem_write(dst_addr, (unsigned int *)(&copy_ch_cfg), sizeof(copy_ch_cfg));
		}
...
void vrx218_local_des_init(unsigned long base)
{
	...
	for (i = 0; i < __US_TC_LOCAL_Q_DES_LIST_NUM; i++) {
		tx_descriptor.dataptr = (PDBRAM_TX_PKT_BUFFER_BASE + (i * 1600));
	...
	for (i = 0; i < __DS_TC_LOCAL_Q_DES_LIST_NUM; i++) {
		rx_descriptor.dataptr = (PDBRAM_RX_PKT_BUFFER_BASE + (i * 1600));
...
void vrx218_ds_desq_cfg_ctxt_init(unsigned long base, int is_bonding,
				  unsigned int vrx218_id, int tc_mode)
{
	unsigned int i, *dst_addr;
...
//  is_bonding = 1, vrx218_id = 0: bonding application, us_bonding_master
//  is_bonding = 1, vrx218_id = 1: bonding application, ds_bonding_master
void vrx218_ds_fragq_desq_cfg_ctxt_init(unsigned long base, int is_bonding,
					unsigned int vrx218_id)
{
	unsigned int i, j, *dst_addr;
	...
	for (i = 0; i < end_fragq_id; i++) {
		...
		dst_addr = (void *) SOC_ACCESS_VRX218_SB((__DS_FRAGQ_CFG_CTXT_BASE +
					  (i * (sizeof(desq_cfg_ctxt_t) / sizeof(unsigned int)))), base);
		dword_mem_write(dst_addr, (unsigned int *)(&fragq_desq_cfg_ctxt), sizeof(desq_cfg_ctxt_t));
		...
		if (vrx218_id == VRX218_US_BONDING_MASTER) {
			...
			for (j = 0; j < fragq_desq_cfg_ctxt.des_num; j++, p_rx_descriptor++) {
				if (j % 4 == 0)
					p_rx_descriptor = (rx_descriptor_t *) KSEG1ADDR(SOC_BOND_DS_DES_SYNC_BUF +
						      ((j / 4) * 64) + 8);
				p_rx_descriptor->own = !fragq_desq_cfg_ctxt.des_in_own_val;
			}
		}
...
void vrx218_cdma_init(unsigned long base, int is_bonding, unsigned int vrx218_id, int cdma_write_data_en, int tc_mode)
{
	...
	*SOC_ACCESS_VRX218_ADDR(VRX218_DMA_PCTRL, base) =
	    ((*SOC_ACCESS_VRX218_ADDR(VRX218_DMA_PCTRL, base) & ~(0x0000003C)) |
	     (0x0000003C));
	...
	*SOC_ACCESS_VRX218_ADDR(VRX218_DMA_CCTRL, base) = *SOC_ACCESS_VRX218_ADDR(VRX218_DMA_CCTRL, base) | 0x00000001;
	...
	if ((is_bonding == 0)
	    || (is_bonding == 1 && vrx218_id == VRX218_US_BONDING_MASTER)) {
...
*************************************************************************/
void __init vrx218_ppe_ptm_init(const ltq_pcie_ep_dev_t * dev,
				const ltq_pcie_ep_dev_t * peer_dev,
				int is_bonding, unsigned int vrx218_id,
				unsigned long sync_buf, int lle_in_sb,
				int cdma_write_data_en)
{
	unsigned long base = (unsigned long)dev->membase;
	...
	vrx218_edma_copy_ch_init(base, is_bonding, vrx218_id,
				 cdma_write_data_en);
...
void __exit vrx218_ppe_ptm_fw_exit(const ltq_pcie_ep_dev_t * dev,
				   int is_bonding, unsigned int vrx218_id)
{
	unsigned int base = (unsigned int)dev->membase;
	volatile unsigned int *ppe_tc_sync =
	    (unsigned int *)xTM_MODE_TO_DSL(base);
...
void __exit vrx218_ppe_ptm_free_mem(const ltq_pcie_ep_dev_t * dev,
				    int is_bonding, unsigned int vrx218_id)
{

#if 0 	/* handled by free_dataptr_fragments */
	unsigned int base = (unsigned int)dev->membase;
	unsigned int i, *dst_addr;
	tx_descriptor_t tx_descriptor;
	...
	if (is_bonding == 0 || vrx218_id == VRX218_US_BONDING_MASTER) {
		...
		for (i = 0; i < 512; i++) {
			...
			skb_to_free = get_skb_pointer(tx_descriptor.dataptr);
			if (skb_to_free != NULL) {
		...
		for (i = 0; i < 64; i++) {
			dst_addr =
			    (void *)
			    SOC_ACCESS_VRX218_SB((__US_OUTQ_DES_LIST_BASE + (i * 2)), base);
			dword_mem_write(&tx_descriptor, dst_addr, sizeof(tx_descriptor));
			skb_to_free = get_skb_pointer(tx_descriptor.dataptr);
			if (skb_to_free != NULL)
				dev_kfree_skb_any(skb_to_free);
