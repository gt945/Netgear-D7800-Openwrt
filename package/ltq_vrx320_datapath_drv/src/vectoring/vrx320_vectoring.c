 ...
 */
#include "vrx320_vectoring_stub.h"
...
static int mei_dsm_cb_func(unsigned int *p_error_vector)
{
    ...
    cb_called++;
    swap_data(p_error_vector, ERB_MAX_PAYLOAD);
    ...
    while ( (skb = skb_list) != NULL ) {
      ...
      //  ((struct erb_head *)skb->data)->length = (unsigned short)(block_size + ERB_BACHCHANNEL_DATA_OFF_LLC);
        ((struct erb_head *)skb->data)->length = htons((unsigned short)(block_size + ERB_BACHCHANNEL_DATA_OFF_LLC));
        if ( skb_list == NULL )
            segment_code |= 0xC0;
...
static int proc_read_dbg(struct seq_file *seq, void *data)
{
    ...
    seq_printf(seq, "cb_called, sent       - %d %d\n", cb_called,erb_sent);
