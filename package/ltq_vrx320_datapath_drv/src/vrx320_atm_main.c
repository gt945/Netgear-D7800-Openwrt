...
#include "vrx320_ppe_atm_ds.h"
#include "vrx320_a1plus_addr_def.h"
#include "vrx320_ppe_fw_const.h"
#include "vrx320_a1plus.h"
#include "unified_qos_ds_be.h"
#include "vrx320_atm_common.h"
 ...
 *  Firmware Registers
 */
#define FW_VER_ID                               ((volatile struct fw_ver_id *)          SOC_ACCESS_VRX218_SB(__FW_VER_ID, g_vrx218_dev.membase))
#define CFG_WRX_HTUTS                           SOC_ACCESS_VRX218_SB(__CFG_WRX_HTUTS, g_vrx218_dev.membase) /*  WAN RX HTU Table Size, must be configured before enable PPE firmware.   */

#define WRX_QUEUE_CONFIG(i)                     ((volatile wrx_queue_config_t *)        SOC_ACCESS_VRX218_SB(__WRX_QUEUE_CONFIG + (i) * 10, g_vrx218_dev.membase))  /*  i < 16  */
#define WTX_QUEUE_CONFIG(i)                     ((volatile wtx_queue_config_t *)        SOC_ACCESS_VRX218_SB(__WTX_QUEUE_CONFIG + (i) * 25, g_vrx218_dev.membase))  /*  i < 15  */

#define HTU_ENTRY(i)                            ((volatile struct htu_entry *)          SOC_ACCESS_VRX218_SB(__HTU_ENTRY_TABLE + (i), g_vrx218_dev.membase))        /*  i < 32  */
#define HTU_MASK(i)                             ((volatile struct htu_mask *)           SOC_ACCESS_VRX218_SB(__HTU_MASK_TABLE + (i), g_vrx218_dev.membase))         /*  i < 32  */
#define HTU_RESULT(i)                           ((volatile struct htu_result *)         SOC_ACCESS_VRX218_SB(__HTU_RESULT_TABLE + (i), g_vrx218_dev.membase))       /*  i < 32  */

#define DSL_WAN_MIB_TABLE                       ((volatile struct dsl_wan_mib_table *)  SOC_ACCESS_VRX218_SB(0x4EF0, g_vrx218_dev.membase))
//  DSL_QUEUE_RX_MIB_TABLE - each queue stands for one PVC
#define DSL_QUEUE_RX_MIB_TABLE(i)               ((volatile struct dsl_queue_mib *)      SOC_ACCESS_VRX218_SB(__WRX_VC_MIB_BASE + (i) * 2, g_vrx218_dev.membase))    /*  i < 16  */
//  DSL_QUEUE_TX_MIB_TABLE, DSL_QUEUE_TX_DROP_MIB_TABLE - multiple queue are attached to one PVC according to WTX_QUEUE_CONFIG
#define DSL_QUEUE_TX_MIB_TABLE(i)               ((volatile struct dsl_queue_mib *)      SOC_ACCESS_VRX218_SB(__WTX_VC_MIB_BASE + (i) * 2, g_vrx218_dev.membase))    /*  i < 16  */
//  Not available in VRX218 A1+
//#define DSL_QUEUE_TX_DROP_MIB_TABLE(i)        ((volatile struct dsl_queue_drop_mib *) SOC_ACCESS_VRX218_SB(0x2FD0 + (i), g_vrx218_dev.membase))   /*  i < 16  */
...
extern int mycount;
extern int mycount_o;

static ltq_pcie_ep_dev_t g_vrx218_dev;
...
static void enable_vrx218_ppe_ema(unsigned int base)
{
	...
	*PMU_PWDCR(base) = rd_val & (~(0x1 << 22));
	...
	dbg("[vrx320] PMU_PWDCR: 0x%08X", *PMU_PWDCR(base));
	dbg("[vrx320] PMU_SR   : 0x%08X", *PMU_SR(base));
...
{
	const char *proc_file = "/proc/vrx320/tfwdbg";
	const char *cmds[] = {
		"help",
		"dessync",
		NULL,
...
{
	volatile desq_cfg_ctxt_t *des_sync_ctxt = (volatile desq_cfg_ctxt_t *)SOC_ACCESS_VRX218_ADDR(SB_BUFFER(addr),g_vrx218_dev.membase);
 ...
 */
static int pp32_download_code(const ltq_pcie_ep_dev_t *dev, int pp32, const u32 *code_src, unsigned int code_dword_len, const u32 *data_src, unsigned int data_dword_len)
{
    ...
    if ( code_dword_len <= CDM_CODE_MEMORYn_DWLEN(0) )
        ...
        set = 0x00;
    dbg("%s: addr(CDM_CFG) = 0x%08x, clr = 0x%02x, set 0x%02x", __FUNCTION__, (unsigned int)CDM_CFG(dev->membase), clr, set);
...
//  vrx218_ppe_atm_init.c
extern void vrx218_ppe_atm_init(const ltq_pcie_ep_dev_t *dev, int qsb_en);
extern void __exit vrx218_ppe_atm_fw_exit(const ltq_pcie_ep_dev_t *dev);
extern void __exit vrx218_ppe_atm_free_mem(const ltq_pcie_ep_dev_t *dev);
//  vrx218_datapath.c
extern int vrx218_atm_datapath_init(const ltq_pcie_ep_dev_t *p_vrx218_dev);
extern void vrx218_atm_datapath_exit(void);

static int __init init_vrx218_module(const ltq_pcie_ep_dev_t *dev)
{
    printk("port        = %d\n",     (unsigned int)dev->membase == VRX320_VIRT_BASE1_VIRT ? 1 : 0);
    printk("irq         = %u\n",     dev->irq);
     ...
     *  init hardware
     */
    reset_ppe((unsigned int)dev->membase); /* AKRONITE FIX */
    /*  activate VRX218 */
    enable_vrx218_ppe_ema((unsigned int)dev->membase);
    /*  set VRX218 PPE clock 288MHz */
    set_vrx218_ppe_clk(288, (unsigned int)dev->membase);
    /* clear mailbox */
    *MBOX_IGU0_ISRC(dev->membase) = 0xFFFFFFFF;
    *MBOX_IGU1_ISRC(dev->membase) = 0xFFFFFFFF;
    *MBOX_IGU0_IER(dev->membase) = 0x00;
    *MBOX_IGU1_IER(dev->membase) = 0x00;
    /*  freeze PP32 */
    pp32_stop(dev);
     ...
     */
    //g_host_desc_base.ds_oam_des_base    = CPHYSADDR(SOC_ACCESS_VRX218_SB(__DS_OAM_DES_LIST_BASE, dev->membase));
    //g_host_desc_base.ds_oam_des_num     = SOC_DS_OAM_DES_NUM;

    //printk("In ATM Init: base->ds_oam_des_base=%#x\n",  g_host_desc_base.ds_oam_des_base);
     ...
     */
    pp32_start(dev);
    setup_dfe_loopback((unsigned long)dev->membase);

    /*
     *  enable mailbox
     */
    *MBOX_IGU0_IER(g_vrx218_dev.membase) = 0x00;    /* not ready to implement flow control yet */
    *MBOX_IGU1_IER(g_vrx218_dev.membase) = 0x2B;    /* not ready to implement swap queue, but enable DS and OAM */

    return PPA_SUCCESS;
}

static void __exit exit_vrx218_module(const ltq_pcie_ep_dev_t *dev)
{
...
static void proc_file_create(void)
{
    ...
    res  = proc_create("showtime",S_IRUGO|S_IWUSR, parent_proc_entry, &g_proc_file_showtime_seq_fops);
}
...
static ssize_t proc_write_dbg(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
    ...
    {
        printk("echo <enable/disable> [err/dbg/assert/rx/tx/swap/all] > /proc/vrx320/dbg\n");
    }
...
static ssize_t proc_write_mem(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
    ...
    if ( *p1 == 'w' )
    {
        ...
        p = (volatile unsigned long *)SOC_ACCESS_VRX218_ADDR(p, g_vrx218_dev.membase);
...
static int proc_read_pp32(struct seq_file *seq, void *v)
{
    ...
    for ( pp32 = 0; pp32 < 2; pp32++ )
    {
        ...
        if ( (*PP32_FREEZE(g_vrx218_dev.membase) & (1 << (pp32 << 4))) != 0 )
        {
            sprintf(str, "freezed");
            f_stopped = 1;
        }
        else if ( PP32_CPU_USER_STOPPED(g_vrx218_dev.membase, pp32) || PP32_CPU_USER_BREAKIN_RCV(g_vrx218_dev.membase, pp32) || PP32_CPU_USER_BREAKPOINT_MET(g_vrx218_dev.membase, pp32) )
        {
            strlength = 0;
            if ( PP32_CPU_USER_STOPPED(g_vrx218_dev.membase, pp32) )
                strlength += sprintf(str + strlength, "stopped");
            if ( PP32_CPU_USER_BREAKPOINT_MET(g_vrx218_dev.membase, pp32) )
                strlength += sprintf(str + strlength, strlength ? " | breakpoint" : "breakpoint");
            if ( PP32_CPU_USER_BREAKIN_RCV(g_vrx218_dev.membase, pp32) )
                strlength += sprintf(str + strlength, strlength ? " | breakin" : "breakin");
            f_stopped = 1;
        }
        else if ( PP32_CPU_CUR_PC(g_vrx218_dev.membase, pp32) == PP32_CPU_CUR_PC(g_vrx218_dev.membase, pp32) )
        {
            ...
            {
                pc_value[i] = PP32_CPU_CUR_PC(g_vrx218_dev.membase, pp32);
                for ( j = 0; j < i; j++ )
        ...
        if ( !f_stopped )
            sprintf(str, "running");
        cur_context = PP32_BRK_CUR_CONTEXT(g_vrx218_dev.membase, pp32);
        seq_printf(seq, "Context: %d, PC: 0x%04x, %s\n", cur_context, PP32_CPU_CUR_PC(g_vrx218_dev.membase, pp32), str);

        if ( PP32_CPU_USER_BREAKPOINT_MET(g_vrx218_dev.membase, pp32) )
        {
            strlength = 0;
            if ( PP32_BRK_PC_MET(g_vrx218_dev.membase, pp32, 0) )
                strlength += sprintf(str + strlength, "pc0");
            if ( PP32_BRK_PC_MET(g_vrx218_dev.membase, pp32, 1) )
                strlength += sprintf(str + strlength, strlength ? " | pc1" : "pc1");
            if ( PP32_BRK_DATA_ADDR_MET(g_vrx218_dev.membase, pp32, 0) )
                strlength += sprintf(str + strlength, strlength ? " | daddr0" : "daddr0");
            if ( PP32_BRK_DATA_ADDR_MET(g_vrx218_dev.membase, pp32, 1) )
                strlength += sprintf(str + strlength, strlength ? " | daddr1" : "daddr1");
            if ( PP32_BRK_DATA_VALUE_RD_MET(g_vrx218_dev.membase, pp32, 0) )
            {
                strlength += sprintf(str + strlength, strlength ? " | rdval0" : "rdval0");
                if ( PP32_BRK_DATA_VALUE_RD_LO_EQ(g_vrx218_dev.membase, pp32, 0) )
                {
                    if ( PP32_BRK_DATA_VALUE_RD_GT_EQ(g_vrx218_dev.membase, pp32, 0) )
                        strlength += sprintf(str + strlength, " ==");
                    else
                        strlength += sprintf(str + strlength, " <=");
                }
                else if ( PP32_BRK_DATA_VALUE_RD_GT_EQ(g_vrx218_dev.membase, pp32, 0) )
                    strlength += sprintf(str + strlength, " >=");
            }
            if ( PP32_BRK_DATA_VALUE_RD_MET(g_vrx218_dev.membase, pp32, 1) )
            {
                strlength += sprintf(str + strlength, strlength ? " | rdval1" : "rdval1");
                if ( PP32_BRK_DATA_VALUE_RD_LO_EQ(g_vrx218_dev.membase, pp32, 1) )
                {
                    if ( PP32_BRK_DATA_VALUE_RD_GT_EQ(g_vrx218_dev.membase, pp32, 1) )
                        strlength += sprintf(str + strlength, " ==");
                    else
                        strlength += sprintf(str + strlength, " <=");
                }
                else if ( PP32_BRK_DATA_VALUE_RD_GT_EQ(g_vrx218_dev.membase, pp32, 1) )
                    strlength += sprintf(str + strlength, " >=");
            }
            if ( PP32_BRK_DATA_VALUE_WR_MET(g_vrx218_dev.membase, pp32, 0) )
            {
                strlength += sprintf(str + strlength, strlength ? " | wtval0" : "wtval0");
                if ( PP32_BRK_DATA_VALUE_WR_LO_EQ(g_vrx218_dev.membase, pp32, 0) )
                {
                    if ( PP32_BRK_DATA_VALUE_WR_GT_EQ(g_vrx218_dev.membase, pp32, 0) )
                        strlength += sprintf(str + strlength, " ==");
                    else
                        strlength += sprintf(str + strlength, " <=");
                }
                else if ( PP32_BRK_DATA_VALUE_WR_GT_EQ(g_vrx218_dev.membase, pp32, 0) )
                    strlength += sprintf(str + strlength, " >=");
            }
            if ( PP32_BRK_DATA_VALUE_WR_MET(g_vrx218_dev.membase, pp32, 1) )
            {
                strlength += sprintf(str + strlength, strlength ? " | wtval1" : "wtval1");
                if ( PP32_BRK_DATA_VALUE_WR_LO_EQ(g_vrx218_dev.membase, pp32, 1) )
                {
                    if ( PP32_BRK_DATA_VALUE_WR_GT_EQ(g_vrx218_dev.membase, pp32, 1) )
                        strlength += sprintf(str + strlength, " ==");
                    else
                        strlength += sprintf(str + strlength, " <=");
                }
                else if ( PP32_BRK_DATA_VALUE_WR_GT_EQ(g_vrx218_dev.membase, pp32, 1) )
                    strlength += sprintf(str + strlength, " >=");
            }
        ...
        if ( f_stopped )
        {
            ...
            {
                for ( j = 0; j < 4; j++ )
                    seq_printf(seq, "   %2d: %08x", i + j * 4, *PP32_GP_CONTEXTi_REGn(g_vrx218_dev.membase, pp32, cur_context, i + j * 4));
                seq_printf(seq, "\n");
            }
        }

        seq_printf(seq, "break out on: break in - %s, stop - %s\n",
                                            PP32_CTRL_OPT_BREAKOUT_ON_BREAKIN(g_vrx218_dev.membase, pp32) ? stron : stroff,
                                            PP32_CTRL_OPT_BREAKOUT_ON_STOP(g_vrx218_dev.membase, pp32) ? stron : stroff);
        seq_printf(seq, "     stop on: break in - %s, break point - %s\n",
                                            PP32_CTRL_OPT_STOP_ON_BREAKIN(g_vrx218_dev.membase, pp32) ? stron : stroff,
                                            PP32_CTRL_OPT_STOP_ON_BREAKPOINT(g_vrx218_dev.membase, pp32) ? stron : stroff);
        seq_printf(seq, "breakpoint:\n");
        seq_printf(seq, "     pc0: 0x%08x, %s\n", *PP32_BRK_PC(g_vrx218_dev.membase, pp32, 0), PP32_BRK_GRPi_PCn(g_vrx218_dev.membase, pp32, 0, 0) ? "group 0" : "off");
        seq_printf(seq, "     pc1: 0x%08x, %s\n", *PP32_BRK_PC(g_vrx218_dev.membase, pp32, 1), PP32_BRK_GRPi_PCn(g_vrx218_dev.membase, pp32, 1, 1) ? "group 1" : "off");
        seq_printf(seq, "  daddr0: 0x%08x, %s\n", *PP32_BRK_DATA_ADDR(g_vrx218_dev.membase, pp32, 0), PP32_BRK_GRPi_DATA_ADDRn(g_vrx218_dev.membase, pp32, 0, 0) ? "group 0" : "off");
        seq_printf(seq, "  daddr1: 0x%08x, %s\n", *PP32_BRK_DATA_ADDR(g_vrx218_dev.membase, pp32, 1), PP32_BRK_GRPi_DATA_ADDRn(g_vrx218_dev.membase, pp32, 1, 1) ? "group 1" : "off");
        seq_printf(seq, "  rdval0: 0x%08x\n", *PP32_BRK_DATA_VALUE_RD(g_vrx218_dev.membase, pp32, 0));
        seq_printf(seq, "  rdval1: 0x%08x\n", *PP32_BRK_DATA_VALUE_RD(g_vrx218_dev.membase, pp32, 1));
        seq_printf(seq, "  wrval0: 0x%08x\n", *PP32_BRK_DATA_VALUE_WR(g_vrx218_dev.membase, pp32, 0));
        seq_printf(seq, "  wrval1: 0x%08x\n", *PP32_BRK_DATA_VALUE_WR(g_vrx218_dev.membase, pp32, 1));
    }
...
static ssize_t proc_write_pp32(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
    ...
    if ( strcasecmp(p, "restart") == 0 )
        *PP32_FREEZE(g_vrx218_dev.membase) &= ~(1 << (pp32 << 4));
    else if ( strcasecmp(p, "freeze") == 0 )
        *PP32_FREEZE(g_vrx218_dev.membase) |= 1 << (pp32 << 4);
    else if ( strcasecmp(p, "start") == 0 )
        *PP32_CTRL_CMD(g_vrx218_dev.membase, pp32) = PP32_CTRL_CMD_RESTART;
    else if ( strcasecmp(p, "stop") == 0 )
        *PP32_CTRL_CMD(g_vrx218_dev.membase, pp32) = PP32_CTRL_CMD_STOP;
    else if ( strcasecmp(p, "step") == 0 )
        *PP32_CTRL_CMD(g_vrx218_dev.membase, pp32) = PP32_CTRL_CMD_STEP;
    else if ( strncasecmp(p, "pc0 ", 4) == 0 )
    {
        ...
        {
            *PP32_BRK_TRIG(g_vrx218_dev.membase, pp32) = PP32_BRK_GRPi_PCn_OFF(0, 0);
            *PP32_BRK_PC_MASK(g_vrx218_dev.membase, pp32, 0) = PP32_BRK_CONTEXT_MASK_EN;
            *PP32_BRK_PC(g_vrx218_dev.membase, pp32, 0) = 0;
        }
        else
        {
            addr = get_number(&p, &rlen, 1);
            *PP32_BRK_PC(g_vrx218_dev.membase, pp32, 0) = addr;
            *PP32_BRK_PC_MASK(g_vrx218_dev.membase, pp32, 0) = PP32_BRK_CONTEXT_MASK_EN | PP32_BRK_CONTEXT_MASK(0) | PP32_BRK_CONTEXT_MASK(1) | PP32_BRK_CONTEXT_MASK(2) | PP32_BRK_CONTEXT_MASK(3);
            *PP32_BRK_TRIG(g_vrx218_dev.membase, pp32) = PP32_BRK_GRPi_PCn_ON(0, 0);
        }
    }
    else if ( strncasecmp(p, "pc1 ", 4) == 0 )
    {
        ...
        if ( strcasecmp(p, "off") == 0 )
        {
            *PP32_BRK_TRIG(g_vrx218_dev.membase, pp32) = PP32_BRK_GRPi_PCn_OFF(1, 1);
            *PP32_BRK_PC_MASK(g_vrx218_dev.membase, pp32, 1) = PP32_BRK_CONTEXT_MASK_EN;
            *PP32_BRK_PC(g_vrx218_dev.membase, pp32, 1) = 0;
        }
        else
        {
            addr = get_number(&p, &rlen, 1);
            *PP32_BRK_PC(g_vrx218_dev.membase, pp32, 1) = addr;
            *PP32_BRK_PC_MASK(g_vrx218_dev.membase, pp32, 1) = PP32_BRK_CONTEXT_MASK_EN | PP32_BRK_CONTEXT_MASK(0) | PP32_BRK_CONTEXT_MASK(1) | PP32_BRK_CONTEXT_MASK(2) | PP32_BRK_CONTEXT_MASK(3);
            *PP32_BRK_TRIG(g_vrx218_dev.membase, pp32) = PP32_BRK_GRPi_PCn_ON(1, 1);
        }
    }
    else if ( strncasecmp(p, "daddr0 ", 7) == 0 )
    {
        ...
        if ( strcasecmp(p, "off") == 0 )
        {
            *PP32_BRK_TRIG(g_vrx218_dev.membase, pp32) = PP32_BRK_GRPi_DATA_ADDRn_OFF(0, 0);
            *PP32_BRK_DATA_ADDR_MASK(g_vrx218_dev.membase, pp32, 0) = PP32_BRK_CONTEXT_MASK_EN;
            *PP32_BRK_DATA_ADDR(g_vrx218_dev.membase, pp32, 0) = 0;
        }
        else
        {
            addr = get_number(&p, &rlen, 1);
            *PP32_BRK_DATA_ADDR(g_vrx218_dev.membase, pp32, 0) = addr;
            *PP32_BRK_DATA_ADDR_MASK(g_vrx218_dev.membase, pp32, 0) = PP32_BRK_CONTEXT_MASK_EN | PP32_BRK_CONTEXT_MASK(0) | PP32_BRK_CONTEXT_MASK(1) | PP32_BRK_CONTEXT_MASK(2) | PP32_BRK_CONTEXT_MASK(3);
            *PP32_BRK_TRIG(g_vrx218_dev.membase, pp32) = PP32_BRK_GRPi_DATA_ADDRn_ON(0, 0);
        }
    }
    else if ( strncasecmp(p, "daddr1 ", 7) == 0 )
    {
        ...
        if ( strcasecmp(p, "off") == 0 )
        {
            *PP32_BRK_TRIG(g_vrx218_dev.membase, pp32) = PP32_BRK_GRPi_DATA_ADDRn_OFF(1, 1);
            *PP32_BRK_DATA_ADDR_MASK(g_vrx218_dev.membase, pp32, 1) = PP32_BRK_CONTEXT_MASK_EN;
            *PP32_BRK_DATA_ADDR(g_vrx218_dev.membase, pp32, 1) = 0;
        }
        else
        {
            addr = get_number(&p, &rlen, 1);
            *PP32_BRK_DATA_ADDR(g_vrx218_dev.membase, pp32, 1) = addr;
            *PP32_BRK_DATA_ADDR_MASK(g_vrx218_dev.membase, pp32, 1) = PP32_BRK_CONTEXT_MASK_EN | PP32_BRK_CONTEXT_MASK(0) | PP32_BRK_CONTEXT_MASK(1) | PP32_BRK_CONTEXT_MASK(2) | PP32_BRK_CONTEXT_MASK(3);
            *PP32_BRK_TRIG(g_vrx218_dev.membase, pp32) = PP32_BRK_GRPi_DATA_ADDRn_ON(1, 1);
        }
    }
    else
    {

        printk("echo \"<command>\" > /proc/vrx320/pp32\n");
        printk("  command:\n");
    ...
    if ( *PP32_BRK_TRIG(g_vrx218_dev.membase, pp32) )
        *PP32_CTRL_OPT(g_vrx218_dev.membase, pp32) = PP32_CTRL_OPT_STOP_ON_BREAKPOINT_ON;
    else
        *PP32_CTRL_OPT(g_vrx218_dev.membase, pp32) = PP32_CTRL_OPT_STOP_ON_BREAKPOINT_OFF;
...
{
    int len = 0;
    u32 base = (u32)g_vrx218_dev.membase;
...
static void __init init_host_desc(void)
{
    ...
    for ( i = 0; i < SOC_ATM_DS_DES_NUM; i++ ) {
        *p++ = 0xB0000000;
        *p++ = 0x00000000;
    }
}
...
//#define SOC_DS_OAM_DES_BASE         CPHYSADDR(SB_BUFFER(DMA_RX_CH1_DESC_PPE_BASE + SOC_ATM_DS_DES_NUM * 2))
#if 0 /* AKRONITE fix */
    ...
    {
	base->us_fastpath_des_base_virt = (unsigned long)pci_alloc_consistent(NULL, SOC_US_FASTPATH_DES_NUM_MAX*8, (dma_addr_t*)&base->us_fastpath_des_base);
        if ( base->us_fastpath_des_base_virt == 0 ) {
            printk("%s failed to allocate us_fastpath_des descriptor base\n", __func__);
            return -ENOMEM;
	}
	memset((void *)base->us_fastpath_des_base_virt, 0, SOC_US_FASTPATH_DES_NUM_MAX*8);
    }

    if ( base->us_cpupath_des_num > 0 )
    {
	base->us_cpupath_des_base_virt = (unsigned long)pci_alloc_consistent(NULL, SOC_US_CPUPATH_DES_NUM_MAX*8, (dma_addr_t*)&base->us_cpupath_des_base);
        if ( base->us_cpupath_des_base_virt == 0 ) {
            printk("%s failed to allocate us_cpupath_des descriptor\n", __func__);
	    pci_free_consistent(NULL, SOC_US_FASTPATH_DES_NUM_MAX*8, (void*)base->us_fastpath_des_base_virt, base->us_fastpath_des_base);
            return -ENOMEM;
	}
	memset((void *)base->us_cpupath_des_base_virt, 0, SOC_US_CPUPATH_DES_NUM_MAX*8);
    }

    if ( base->ds_des_num > 0 )
    {
	base->ds_des_base_virt = (unsigned long)pci_alloc_consistent(NULL, SOC_ATM_DS_DES_NUM_MAX*8, (dma_addr_t*)&base->ds_des_base);
        if ( base->ds_des_base_virt == 0 ) {
            printk("%s failed to allocate ds_des_base descriptor\n", __func__);
       	    pci_free_consistent(NULL, SOC_US_FASTPATH_DES_NUM_MAX*8, (void*)base->us_fastpath_des_base_virt, base->us_fastpath_des_base);
       	    pci_free_consistent(NULL, SOC_US_CPUPATH_DES_NUM_MAX*8, (void*)base->us_cpupath_des_base_virt, base->us_cpupath_des_base);
            return -ENOMEM;
	}
	memset((void *)base->ds_des_base_virt, 0, SOC_ATM_DS_DES_NUM_MAX*8);
    }

    if ( base->ds_oam_des_num > 0 )
    {
	base->ds_oam_des_base_virt = (unsigned long)pci_alloc_consistent(NULL, SOC_DS_OAM_DES_NUM_MAX*8, (dma_addr_t*)&base->ds_oam_des_base);
        if ( base->ds_oam_des_base_virt == 0 ) {
            printk("%s failed to allocate ds_oam_des_base descriptor\n", __func__);
       	    pci_free_consistent(NULL, SOC_US_FASTPATH_DES_NUM_MAX*8, (void*)base->us_fastpath_des_base_virt, base->us_fastpath_des_base);
       	    pci_free_consistent(NULL, SOC_US_CPUPATH_DES_NUM_MAX*8, (void*)base->us_cpupath_des_base_virt, base->us_cpupath_des_base);
       	    pci_free_consistent(NULL, SOC_ATM_DS_DES_NUM_MAX*8, (void*)base->ds_des_base_virt, base->ds_des_base);
            return -ENOMEM;
	}
	memset((void *)base->ds_oam_des_base_virt, 0, SOC_DS_OAM_DES_NUM_MAX*8);
    }
...
static int __init ppe_init(void)
{
	...
	printk("vrx320: loading PPE driver and firmware (ATM)\n");

	g_host_desc_base.us_fastpath_des_num = SOC_US_FASTPATH_DES_NUM;
	g_host_desc_base.us_cpupath_des_num  = SOC_US_CPUPATH_DES_NUM;
	g_host_desc_base.ds_des_num          = SOC_ATM_DS_DES_NUM;
	g_host_desc_base.ds_oam_des_num      = SOC_DS_OAM_DES_NUM; //ALEX initially it was 0
	...
	for ( i = 0; i < dev_num; i++ ) {
		if ( ltq_pcie_ep_dev_info_req(i, IFX_PCIE_EP_INT_PPE, &g_vrx218_dev) ) {
...
static void __exit ppe_exit(void)
{
	...
	if(base->ds_oam_des_base_virt)
		pci_free_consistent(NULL, SOC_DS_OAM_DES_NUM_MAX*8, (void*)base->ds_oam_des_base_virt, base->ds_oam_des_base);

	if ( g_vrx218_idx >= 0 ) {
		exit_vrx218_module(&g_vrx218_dev);

		ltq_pcie_ep_dev_info_release(g_vrx218_idx);
		g_vrx218_idx = -1;
	}
}
