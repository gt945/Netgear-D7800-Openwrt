...
#ifndef AKRONITE_BOARD
#define AKRONITE_BOARD
//#include <asm/chacheflush.h>
#endif
...
#else
    skb_dataptr = (dataptr - 4) | 0xa0000000;
#endif
    //printk("**********%s:skb_dataptr=%#x****\n", __func__, skb_dataptr);
    skb = *(struct sk_buff **)skb_dataptr;

    //__get_skb_from_dbg_pool(skb, func_name, line_num);

    //ASSERT((unsigned int)skb >= 0x80000000, "%s:%d: invalid skb - skb = %#08x, dataptr = %#08x", func_name, line_num, (unsigned int)skb, dataptr);
    //ASSERT((((unsigned int)skb->data & (0x1FFFFFFE ^ (DMA_ALIGNMENT - 1))) | 0xa0000000) == (dataptr | 0xa0000000), "%s:%d: invalid skb - skb = %#08x, skb->data = %#08x, dataptr = %#08x", func_name, line_num, (unsigned int)skb, (unsigned int)skb->data, dataptr);
...
dma_addr_t alloc_dataptr_skb(void)
{
	...
	panic("ATM: %s buffer allocation failure\n",__func__);
    }
...
#ifdef AKRONITE_BOARD
void *alloc_dataptr_fragment(unsigned int bytes)
#else
...
void vrx218_ds_soc_des_init(unsigned long base, int is_bonding, unsigned int vrx218_id)
{
...
#ifdef AKRONITE_BOARD
	//unsigned int ddr_base = (rx_descriptor.dataptr & 0xe0000000) + 0x20000000;
    ...
    if (is_bonding == 0) {
...
#ifdef AKRONITE_BOARD
            //rx_descriptor.dataptr = virt_to_dma(NULL, alloc_dataptr_fragment(1600));
		//tmp_skb = alloc_skb_tx(1600);
	    rx_descriptor.dataptr = dma_map_single(NULL, alloc_dataptr_fragment(1600), 1600, DMA_FROM_DEVICE);
	    //rx_descriptor.dataptr = dma_map_single(NULL, tmp_skb->data, 1600, DMA_FROM_DEVICE);
#else
        ...
        for (i=0; i<32; i++) {
#ifdef AKRONITE_BOARD
            //rx_descriptor.data_ptr = virt_to_dma(NULL, alloc_dataptr_fragment(560));
	    rx_descriptor.dataptr = dma_map_single(NULL, alloc_dataptr_fragment(560), 560, DMA_FROM_DEVICE);
#else
...
void vrx218_ds_desq_cfg_ctxt_init(unsigned long base, int is_bonding, unsigned int vrx218_id, int tc_mode)
{
    ...
    ds_desq_cfg_ctxt.des_num         = SOC_ATM_DS_DES_NUM;      //  max: 64 (for PTM)
    ...
    if (tc_mode == 1) {
        ds_desq_cfg_ctxt.des_num         = SOC_ATM_DS_DES_NUM;  //  max: 32
        ds_desq_cfg_ctxt.des_base_addr   = __DS_PKT_DES_LIST_BASE;
        dst_addr = (void *)SOC_ACCESS_VRX218_SB(__DS_PKT_DESQ_CFG_CTXT, base);
    }
#endif
...
void vrx218_cdma_init(unsigned long base, int is_bonding, unsigned int vrx218_id, int cdma_write_data_en, int tc_mode)
{
    ...
    *SOC_ACCESS_VRX218_ADDR(VRX218_DMA_CCTRL, base) = *SOC_ACCESS_VRX218_ADDR(VRX218_DMA_CCTRL, base) | 0x00000001;
    ...
    if (cdma_write_data_en == 1) {
        ...
        *SOC_ACCESS_VRX218_ADDR(VRX218_DMA_CDLEN, base) = SOC_ATM_DS_DES_NUM * 2;
    ...
    if (tc_mode == 1) {
        ...
        *SOC_ACCESS_VRX218_SB(((__DES_SYNC_CFG_CTXT + 96) + 11), base) = ((unsigned int)VRX218_ACCESS_VRX218_SB(__DES_SYNC_CFG_CTXT + 96 + 16) & 0x1FFFFFFF);
        *SOC_ACCESS_VRX218_ADDR(VRX218_DMA_CS, base) = 0x6;
...
void vrx218_ds_oam_desq_cfg_ctxt_init(unsigned long base)
{
    ...
    ds_desq_cfg_ctxt.fast_path       = 0;
    ds_desq_cfg_ctxt.mbox_int_en     = 0;
    ds_desq_cfg_ctxt.des_sync_needed = 1;   //  not copied to host
...
#ifdef AKRONITE_BOARD
        //rx_descriptor.dataptr = virt_to_dma(NULL, alloc_dataptr_fragment(64));
	rx_descriptor.dataptr = dma_map_single(NULL, alloc_dataptr_fragment(64), 64, DMA_FROM_DEVICE);
#else
        rx_descriptor.data_ptr = (unsigned int)alloc_dataptr_fragment(64);
#endif
...
void vrx218_atm_us_qos_des_init(unsigned long base)
{
    ...
    for (i=0; i<512; i++) {
#ifdef AKRONITE_BOARD
	tx_descriptor.dataptr = (unsigned int)alloc_dataptr_skb();
#else
	tx_descriptor.data_ptr = (unsigned int)alloc_dataptr_skb();
#endif
...
void vrx218_atm_des_sync_init(unsigned long base)
{
    ...
    des_sync_cfg_ctxt.max_polling_intv   = 15; //4;
    des_sync_cfg_ctxt.desq_cfg_ctxt      = __US_FP_INQ_DES_CFG_CTXT; // Here we re-use for OAM US.
    //des_sync_cfg_ctxt.desq_cfg_ctxt      = 0; //__US_FP_INQ_DES_CFG_CTXT; //To stop syncing US FP DESC.
    ...
    //des_sync_cfg_ctxt.us_des_polling_needed = 0;
    des_sync_cfg_ctxt.max_polling_intv   = 15; //1;
...
*****************************************************************/
void vrx218_ppe_atm_init(const ltq_pcie_ep_dev_t *dev, int qsb_en)
{
...
void __exit vrx218_ppe_atm_fw_exit(const ltq_pcie_ep_dev_t *dev)
{
...
void __exit vrx218_ppe_atm_free_mem(const ltq_pcie_ep_dev_t *dev)
{
#if 0
    unsigned int base = (unsigned int) dev->membase;
    unsigned int i, *dst_addr;
...
#endif //We call all these mem free operations in free_dataptr_fragments() now.
