...
int set_region(u16 host_region_number)
{
	...
	/* Allowed Region Number range: 0x0001 ~ 0x000f */
	if (!(host_region_number >= 0x0001 &&
	      host_region_number <= 0x000f)) {
		printf("Wrong Region Number: 0x%hx\n", host_region_number);
		return 1;
	}
#ifndef DNI_NAND
	memcpy(sectorBuff, (void *)BOARDCAL, CONFIG_SYS_FLASH_SECTOR_SIZE);
...
int do_rnset(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	for (i=0; i<REGION_NUMBER_LENGTH; ++i)  /* Every digit should be in range 0~9 */
	{
		if (!((rn[i]>=0x30) && (rn[i]<=0x39)))    /*rn is 01~09*/
		{
			puts ("Input 01 ~ 15 for REGION Number\n");
			break;
		}
	}
	if (i < REGION_NUMBER_LENGTH)  /* One digit is not in range 0~9 */
		return 1;
