...
int checkboard(void)
{
	printf("U-boot 2012.07 dni1 V0.1 for DNI HW ID: 29764881 NOR flash 0MB NAND flash 128MB RAM 256MB 1st Radio 4x4 2nd Radio 4x4\n");
	return 0;
}
...
void board_upgrade_string_table(unsigned char *load_addr, int table_number, unsigned int file_size)
{
    ...
    snprintf(cmd, 128, "ubi read 0x%lx %s 0x%zx",
             ...
             CONFIG_SYS_STRING_TABLE_PARTITION_NAME, read_size);
    ret = run_command(cmd, 0);
